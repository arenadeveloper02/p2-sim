...
Each page is one primary archetype + short capabilities + optional regions. Representation (`auto | table | cards | list | kanban | timeline`) picks the collection body. `kanban` has no catalog type — it degrades to grouped Repeat/Table. `timeline` uses catalog `Timeline`. The spec must not invent `Kanban` or `List` components. Catalog `Map`, `Tree`, `Carousel`, and `Timeline` exist for pins, nested folders, cycling galleries, and chronological spines. Do not emit `detail + results + dashboard` as peer jobs on one page. Do not add pages, history, stats, or modules the blueprint omitted.
