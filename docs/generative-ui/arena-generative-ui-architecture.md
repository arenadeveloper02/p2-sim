...
`planArenaGenerativeStructuredBrief` consumes that intent (when present) and emits title, purpose, audience, an app-level archetype, per-page shapes, optional `entity`, optional `representation`, optional `pages[].secondary`, sitemap, actions, `capabilities[]` (at most five planned), optional `designIntent`, optional `informationHierarchy`, and optional `interactionModel`. The planner sees a compact Design System blurb (two surfaces, host-owned color/type/radius, density `compact|comfortable|roomy`, spacing tokens on gap/padding) so `designIntent` is classified with token context — it still must not emit component types or a manifest. Legacy stored `processing` wait tags fold into `capabilities` so old drafts still edit. Intent is nested on the same jsonb (`structured_brief.intent`) — no DB migration. Unknown designIntent / hierarchy / interaction axes are dropped (fail-open); `spacious` density aliases to `roomy`. Unknown `representation` fails open to `auto`.

**Archetype is the job, not the visual layout.** Composition is primary shape + optional secondary region + 0–5 capabilities. Representation (`auto | table | cards | list | kanban | timeline`) picks the collection body. `kanban` / `timeline` have no catalog types — they degrade to grouped or dated Repeat/Table. The spec must not invent `Kanban`, `Timeline`, or `List` components.

Wait tags (`long-running`, `streaming`, `multi-step`, `cancellable`, `progress`) apply to any page shape. Product tags (`search`, `filter`, `sort`, `pagination`, `grouping`, `date-range`, `refresh`, `drill-down`, `selection`, `detail`, `detail-drawer`, `analyze`, `drawer`, `modal`, `create`, `edit`, `delete`, `back`, `skip`, `review`) are short when/how recipes. `detail` opens one entity (navigate, drawer, modal, or inline); `detail-drawer` is the keep-list-visible specialization; `analyze` is a generate/analyze CTA whose destination is a results page or secondary results. Stored `editable` aliases to `edit`. Host inference (does not count toward the planned five): workflow binding → `long-running`; `stream: true` → `streaming`; `binding.pagination` → `pagination`. `short` is not a capability (omit wait modules). `wait: working-card` on interactionModel only when a wait capability is set. Do not plan `generate` / `export` / `table` / `chart` / `kanban` as capabilities.

### Page shapes

The app has one primary `archetype` (the entry job). **Each sitemap page also declares `pages[].archetype`**. Mixed apps are normal: home = `task` and destination = `results`; home = `collection` and record = `detail`. Extra pages are extra shapes, not a second app-level archetype. Formula: one shape + optional secondary + 0–5 planned capabilities + `representation` + `BindingLayoutPlan` + `designIntent`.

A non-workspace page may set `pages[].secondary { role, archetype }` for a supporting pane (Columns, or Drawer when `detail` / `detail-drawer`). That is how a projects list sits beside project details without a `project-workspace` archetype. Workspace remains the three-region shell.

| Shape | Core question | Slots |
|---|---|---|
| `collection` | How do I browse/manage many things? | Header → Toolbar → collection body (representation decides Table / Cards / List / grouped Repeat) → optional secondary |
| `detail` | How do I understand one thing? | Header, facts, data-model sections, related, actions |
| `task` | How do I provide information to accomplish something? | Header, context, Form or SearchField, one primary action. Results are optional |
...
| `workspace` | Where do I work across several related things at once? | Catalog `Workspace`: navigator / primary / inspector. Sync via `selectedId`. Host collapses inspector, then navigator |
