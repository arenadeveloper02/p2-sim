...
/**
 * Pre-split generic-query operation ids from the previous Semrush block.
 * Mapped onto the dedicated Analytics / Position Tracking tools so stored
 * workflows keep executing after this merge.
 */
const LEGACY_SEMRUSH_OPERATIONS: Record<string, string> = {
  url_organic: 'semrush_url_organic_keywords',
  domain_organic: 'semrush_domain_organic_keywords',
  domain_organic_organic: 'semrush_domain_organic_competitors',
...
export const SemrushBlock: BlockConfig<SemrushResponse> = {
  ...
  longDescription:
    'Query the Semrush SEO API for domain, subdomain, and URL traffic overviews, organic and paid keywords, ad and product listing creatives, competitor sets, keyword research, backlink profiles, and Position Tracking organic positions. Requires a Semrush subscription with API units.',
  ...
  canvasPresentation: {
    ...
    sentences: {
      byOperation: {
        ...
        semrush_organic_positions: [
          { text: 'Read tracked organic positions for', field: 'campaignId', core: true },
        ],
      },
    },
  },
  subBlocks: [
    {
      ...
      options: [
        ...
        { label: 'Organic Positions (Position Tracking)', id: 'semrush_organic_positions' },
      ],
    ...
    },
    {
      ...
      condition: {
        field: 'operation',
        value: [...URL_OPERATIONS, 'semrush_organic_positions'],
      },
      required: {
        field: 'operation',
        value: [...URL_OPERATIONS, 'semrush_organic_positions'],
...
Return ONLY the expression - no explanations, no quotes, no extra text.`,
    ...
    },
    {
      ...
      required: { field: 'operation', value: 'semrush_organic_positions' },
      condition: { field: 'operation', value: 'semrush_organic_positions' },
    },
    {
      id: 'dateBegin',
      title: 'Date begin',
  ...
  tools: {
    ...
    config: {
      tool: (params) => {
        const raw = params.operation as string | undefined
        const operation = raw ? (LEGACY_SEMRUSH_OPERATIONS[raw] ?? raw) : undefined
      ...
      params: (params) => {
        ...
        else if (params.displayLimit && operation !== 'semrush_organic_positions') {
          result.limit = Number(params.displayLimit)
        }
        ...
        if (!params.url && typeof params.trackingUrl === 'string' && params.trackingUrl) {
          result.url = params.trackingUrl
        }
        return result
      },
    },
  ...
  inputs: {
    ...
    dateBegin: { type: 'string', description: 'Position Tracking start date (YYYYMMDD)' },
    dateEnd: { type: 'string', description: 'Position Tracking end date (YYYYMMDD)' },
    linktypeFilter: { type: 'string', description: 'Position Tracking link-type filter' },
    displayLimit: { type: 'string', description: 'Position Tracking row limit' },
    displayOffset: { type: 'string', description: 'Position Tracking pagination offset' },
    displayTags: { type: 'string', description: 'Position Tracking tag filter' },
    displayTagsCondition: { type: 'string', description: 'Position Tracking tag condition' },
    topFilter: { type: 'string', description: 'Position Tracking top-N position filter' },
    useVolume: { type: 'string', description: 'Position Tracking volume level' },
    businessName: { type: 'string', description: 'Google Business Profile name' },
    serpFeatureFilter: { type: 'string', description: 'Position Tracking SERP feature filter' },
  },
  ...
  outputs: {
    ...
    data: {
      type: 'json',
      description: 'Position Tracking keyword rows with rankings and visibility',
    },
    totalRows: {
      type: 'number',
      description: 'Total Position Tracking rows in the report',
    },
    reportType: {
      type: 'string',
      description: 'Report type returned by Position Tracking or a generic query',
    },
    raw: {
      type: 'json',
      description: 'Raw Position Tracking API response',
    },
  },
}
