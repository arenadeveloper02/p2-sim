...
describe('ArenaGenerativeUiBlock field tooltips', () => {
  ...
  it('gives User Input extra rows and previews the original brief in Edit', () => {
    ...
    expect(userInput?.wandConfig?.prompt).toContain('{compiled}')
    expect(userInput?.wandConfig?.prompt).toContain('REPAIR')
    expect(userInput?.wandConfig?.prompt).toContain('Do not invent API keys')
    expect(userInput?.wandConfig?.prompt).toContain('closest declared key')
    expect(userInput?.wandConfig?.prompt).toContain('Original intent')
