...
const logger = createLogger('GTMMetricsAnalyzeAPI')

/**
 * POST /api/gtm-metrics/analyze
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info('🚀 GTM METRICS API CALLED!', {
      question,
      timeframe,
      accounts,
      url: request.url,
    ...
    logger.info('Google Ads data fetched', {
      ...
      totalSpends: googleAdsData.accounts.reduce((sum: number, acc: any) => sum + acc.spends, 0),
    })

    // Step 3: Fetch comparison period data if requested
    let previousPeriodData
    if (includeComparison && includeComparison !== 'none') {
      const comparisonRange = getComparisonDateRange(dateRange, includeComparison)
      previousPeriodData = await fetchGoogleAdsData(comparisonRange, accounts)
    ...
    const gtmInput: GTMMetricsInput = {
      ...
      previousPeriod: previousPeriodData
        ? {
            accounts: previousPeriodData.accounts,
          }
        : undefined,
    }
    ...
    // Step 6: Generate AI insights (placeholder for now)
...
1. **Budget Optimization**: ${
      gtmMetrics.topPerformingAccounts.length > 0
...
2. **Risk Mitigation**: ${
      gtmMetrics.atRiskAccounts.length > 0
...
3. **Growth Opportunities**: ${
      gtmMetrics.momGrowth && gtmMetrics.momGrowth.revenue > 0
...
*This analysis is based on Google Ads performance data and GTM metrics calculations.*`
    ...
    logger.info('🎉 GTM Metrics Analysis completed successfully')
    ...
    logger.info('🔍 GTM API Response Structure:', {
    ...
    logger.error('GTM Metrics Analysis failed', { error })
