...
⚠️ **CRITICAL - ASSET RESOURCES EXCEPTION:**
**NEVER use segments.date with campaign_asset, asset, or asset_group_asset resources!**
- These resources DO NOT support date segmentation
- Queries for assets, sitelinks, callouts, etc. MUST NOT include segments.date in SELECT or WHERE
- Assets are current state only - they have no historical time-series data
- If user asks for "current assets" or "all assets", do NOT add any date filtering
