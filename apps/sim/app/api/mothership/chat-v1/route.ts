...
import { resolveOrCreateChat } from '@/lib/copilot/chat-lifecycle-v1'
...
export async function POST(req: NextRequest) {
  ...
  const { userId } = MothershipMessageSchema.parse(body)
  ...
  try {
    ...
    if(userId) {
      session = { user: { id: userId, createdAt: new Date(), updatedAt: new Date(), email: 'anonymous@example.com', emailVerified: false, name: 'Anonymous' } ,session: { id: 'anonymous-session', userId: userId, expiresAt: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000), createdAt: new Date(), updatedAt: new Date(), token: 'anonymous-token', ipAddress: null, userAgent: null } }
    }
    if (!session?.user?.id) {
    ...
    logger.error(
      appendCopilotLogContext('Received mothership chat-duplicate start request', {
    ...
    logger.error(
      appendCopilotLogContext('Error handling mothership chat-duplicate', {
        requestId: tracker.requestId,
