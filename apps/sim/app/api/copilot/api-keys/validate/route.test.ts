...
describe('POST /api/copilot/api-keys/validate billing protocols', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    resetDbChainMock()
    // Protocol admission is a hosted Go↔Sim contract; self-hosted mothership
    // metering is covered in the suite below.
  ...
  it('rejects markerless admission only when protocol-required is explicitly enabled', async () => {
    setEnvFlags({ isHosted: true, isCopilotBillingProtocolRequired: true })
    const res = await POST(request(OLD_GO_HOSTED_VALIDATE_BODY))
...
describe('POST /api/copilot/api-keys/validate self-hosted mothership', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    resetDbChainMock()
    setEnvFlags({ isHosted: false, isCopilotBillingProtocolRequired: false })
    mockCheckInternalApiKey.mockReturnValue({ success: true })
    queueTableRows(schemaMock.user, [{ id: 'user-1' }])
    mockResolveLegacyV0BillingAttribution.mockResolvedValue(null)
    mockCheckSelfHostedMothershipUsageLimits.mockResolvedValue({
      isExceeded: false,
      currentUsage: 10,
      limit: 100,
  ...
  it('meters mothership cost only and skips hosted attributed limits', async () => {
    const res = await POST(request(OLD_GO_WORKSPACELESS_VALIDATE_BODY))
  ...
  it('returns 402 when self-hosted mothership usage is exceeded', async () => {
    mockCheckSelfHostedMothershipUsageLimits.mockResolvedValue({
      isExceeded: true,
      currentUsage: 120,
      limit: 100,
