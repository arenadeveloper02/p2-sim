    ...
    async (span) => {
      try {
        ...
        if (!isHosted) {
          ...
          const mothershipUsage = await checkMothershipUsageLimits(userId, workspaceId)
          span.setAttribute(TraceAttr.BillingUsageExceeded, mothershipUsage.isExceeded)

          if (mothershipUsage.isExceeded) {
            span.setAttribute(
              TraceAttr.CopilotValidateOutcome,
              CopilotValidateOutcome.UsageExceeded
            )
