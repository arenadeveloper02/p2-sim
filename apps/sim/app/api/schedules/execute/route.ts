...
function isStreamingExecution(
  ...
  return (
    typeof result === 'object' && result !== null && 'stream' in result && 'execution' in result
  )
}
        ...
        try {
          const executionSuccess = await (async () => {
            ...
            try {
              ...
              // Now process block states using the merged env vars (which includes system fallbacks)
              ...
              )) as ExecutionResult | StreamingExecution

              const executionResult: ExecutionResult = isStreamingExecution(result)
                ? result.execution
                : result
