...
vi.mock('@/lib/chat-deployments/golden-queries', () => ({
  syncWorkflowGoldenQueriesFromCustomizations: mocks.syncGoldenQueries,
}))
...
describe('Chat API Route', () => {
  ...
  beforeEach(() => {
    ...
    mocks.syncGoldenQueries.mockResolvedValue(undefined)
  ...
  describe('POST', () => {
    ...
    it('persists golden queries after a successful deploy', async () => {
      queueChatLookups(null, null)

      await post({
        ...validBody,
        customizations: {
          ...validBody.customizations,
          goldenQueries: ['  How do I reset my password?  ', ''],
        },
      })

      expect(mocks.syncGoldenQueries).toHaveBeenCalledWith({
        workflowId: WORKFLOW_ID,
        userId: 'admin-1',
        goldenQueries: ['  How do I reset my password?  ', ''],
      })
    ...
    it('does not persist golden queries when the field is omitted', async () => {
      queueChatLookups(null, null)
      ...
      expect(mocks.syncGoldenQueries).toHaveBeenCalledWith({
        workflowId: WORKFLOW_ID,
        userId: 'admin-1',
        goldenQueries: undefined,
      })
    })

    it('forwards department, deployment type, and redirect URL', async () => {
      queueChatLookups(null, null)
    ...
    it('rejects an app deployment without a redirect URL', async () => {
      queueChatLookups(null, null)
      ...
      expect(response.status).toBe(400)
      expect((await response.json()).error).toBe(
        'Redirect URL is required when deploying as an app'
      )
