...
const {
  ...
  mockResolveMothershipChatAttribution: vi.fn(),
...
vi.mock('@/lib/billing/core/mothership-chat-attribution', () => ({
  resolveMothershipChatAttributionFromMessageId: mockResolveMothershipChatAttribution,
}))
...
describe('POST /api/billing/update-cost — workspaceId attribution', () => {
  beforeEach(() => {
    ...
    mockResolveMothershipChatAttribution.mockResolvedValue(undefined)
  ...
  it('records keyless legacy callbacks through the append-only fallback', async () => {
    const res = await POST(
