...
async function updateCostInner(req: NextRequest, span: Span): Promise<NextResponse> {
  ...
  try {
    ...
    /**
     * Go may omit chat/run IDs or send IDs from another deployment. Recover
     * local attribution from the stable message/stream ID so usage analytics
     * can join the cumulative charge without weakening immutable payer binding.
     */
    if (!attributedChatId || !attributedRunId) {
    ...
    /** Copilot callbacks are user-triggered, so stamp actor lineage explicitly. */
