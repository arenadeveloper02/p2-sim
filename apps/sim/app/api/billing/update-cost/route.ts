...
/**
 * Resolves a request workspace to a local row before writing the optional
 * usage-log foreign key. Unknown workspaces remain unattributed rather than
 * causing a real callback charge to fail its insert.
 */
...
async function updateCostInner(req: NextRequest, span: Span): Promise<NextResponse> {
  ...
  try {
    ...
    const resolvedWorkspaceId = isDirectProtocol
      ? undefined
      : (billingAttribution?.workspaceId ?? attributedWorkspaceId)
    ...
    if (idempotencyKey) {
      ...
      logger.info(`[${requestId}] Cumulative cost top-up`, {
        ...
        billed: result.billed,
        addedCost: scaleUsageLogCost(cost),
        multiplier: getUsageLogCostMultiplier(),
      })
    } else {
      await recordUsage({
        userId,
