...
function validateDateFiltering(response: GAQLResponse): GAQLResponse {
  ...
  // Snapshot-only resources do not support segments.date - skip injection
  if (/FROM\s+(asset|campaign_asset|asset_group_asset|campaign_criterion|geo_target_constant|shopping_product)\b/i.test(response.gaql_query)) {
    return response
  }
...
export async function generateGAQLQuery(userPrompt: string): Promise<GAQLResponse> {
  try {
    ...
    // Sanitize change_event queries before generic date validation
