...
/**
 * Resolves a knowledge base identifier: returns UUIDs unchanged, otherwise looks up by name.
 */
...
export const POST = withRouteHandler(async (request: NextRequest) => {
  ...
  try {
    ...
    let knowledgeBaseIds = Array.isArray(validatedData.knowledgeBaseIds)
      ? validatedData.knowledgeBaseIds
      : [validatedData.knowledgeBaseIds]

    if (validatedData.advancedMode === true) {
      const resolvedIds = await Promise.all(
        knowledgeBaseIds.map((id) => resolveKnowledgeBaseId(id))
      )
      knowledgeBaseIds = resolvedIds.filter((id): id is string => id !== null)
      logger.info(`[${requestId}] Knowledge base search in advanced mode (name → id)`, {
        knowledgeBaseIds,
        hasQuery: Boolean(validatedData.query?.trim()),
        hasTagFilters: Boolean(validatedData.tagFilters?.length),
      })
    }

    if (validatedData.advancedMode === true && knowledgeBaseIds.length === 0) {
      return NextResponse.json(
        {
          error:
            'No knowledge bases matched the given names or IDs. Check names are exact and bases are not deleted.',
        },
        { status: 404 }
      )
    }
    ...
    if (kbIds.length > 0) {
      const kbWorkspaceRows = await db
        ...
        .limit(500)
      for (const row of kbWorkspaceRows) {
