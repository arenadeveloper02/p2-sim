...
import { arenaGenerativeGenerateContract } from '@/lib/api/contracts/arena-generative-apps'
...
export const POST = withRouteHandler(async (request: NextRequest) => {
  ...
  if (!auth.success || !auth.userId) {
    logger.warn(`[${requestId}] Unauthorized generate attempt: ${auth.error}`)
  ...
  const parsed = await parseArenaGenerativeUiRequest(arenaGenerativeGenerateContract, request)
  ...
  if (!response.success) {
    logger.warn(`[${requestId}] Generation failed`, { error: response.error })
    return NextResponse.json(response, { status: 500 })
  }

  logger.info(`[${requestId}] Generation succeeded`, { draftId: response.output.draftId })
