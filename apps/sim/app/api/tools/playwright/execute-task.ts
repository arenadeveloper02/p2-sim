...
import type {
  PlaywrightAgentStep,
  PlaywrightRunTaskOutput,
  PlaywrightRunTaskParams,
} from '@/tools/playwright/types'

const logger = createLogger('PlaywrightExecuteTask')
 ...
 * Server-only — must not be imported from client-bundled tool configs.
 */
export async function executePlaywrightTask(
  params: PlaywrightRunTaskParams
): Promise<PlaywrightExecuteResult> {
  ...
  if (!task) {
    return {
      success: false,
      output: { success: false, output: null, url: null, steps: [] },
    ...
    for (let i = 0; i < maxSteps; i++) {
      ...
      if (action.action === 'done') {
        ...
        steps.push({
          number: steps.length + 1,
          action: 'done',
          detail: typeof action.output === 'string' ? action.output : JSON.stringify(action.output),
          url: page.url(),
        })
        break
      }
