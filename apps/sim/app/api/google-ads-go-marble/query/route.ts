...
const logger = createLogger('GoogleAdsAPI-GoMarble')
...
async function listAccounts(): Promise<any> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Error listing accounts: ${response.status} ${response.statusText} - ${error}`
      )
    }
...
async function runKeywordPlanner(params: {
  ...
  try {
    ...
    const startMonthFinal =
      start_month && validMonths.includes(start_month.toUpperCase())
        ? start_month.toUpperCase()
        : 'JANUARY'
    const endYearFinal = end_year || currentYear
    const endMonthFinal =
      end_month && validMonths.includes(end_month.toUpperCase())
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Error executing request: ${response.status} ${response.statusText} - ${errorText}`
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info(`Google Ads API request (Go Marble): tool=${tool}`)
    ...
    logger.error(`Google Ads API error (Go Marble): ${error}`)
    return NextResponse.json(
      {
