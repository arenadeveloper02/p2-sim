...
function MessageContentInner({
  ...
  const showTrailingLiveStatus = shouldShowTrailingLiveStatus({
    isStreaming,
    liveStatus,
    hasTrailingContent,
    hasRunningWork,
  ...
  const showShimmer =
    ...
    (segments.length === 0 ||
      ...
      (showTrailingLiveStatus && Boolean(liveStatus?.trim())))

  const pendingLabel = liveStatus?.trim() ? liveStatus : (thinkingLabel ?? 'Thinking…')
    ...
    <div>
      ...
      {thinkingExpanded && isLast ? (
        ...
        <div aria-hidden={!showShimmer} className={TAIL_REGION_CLASSES}>
          ...
          >
            <PendingTagIndicator label={pendingLabel} />
          </div>
        </div>
