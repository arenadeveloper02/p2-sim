  ...
  React.useEffect(() => {
    if (fieldAdvancedMode) {
      if (typeof selectedValue === 'object' && selectedValue?.clientId) {
        ...
        const clientName = selectedValue.name || selectedValue.customDisplayValue || ''
        if (!isPreview && !disabled && clientName) {
          setStoreValue(clientName)
        }
      } else if (
        ...
        if (matchedClient && !isPreview && !disabled) {
          setStoreValue(matchedClient.name)
        }
      }
    } else {
