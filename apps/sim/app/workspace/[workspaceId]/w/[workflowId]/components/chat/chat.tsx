...
type ChatFileWithDataUrl = ChatFile & {
  dataUrl?: string
}
...
export function Chat() {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      const combinedChatFiles: ChatFileWithDataUrl[] = [
        ...chatFiles,
      ...
      const localAttachments = await processFileAttachments(combinedChatFiles)

      const fileArray: Array<WorkflowAttachmentInput & { id?: string; dataUrl?: string }> =
      ...
      const uploadedMessageAttachments = toChatMessageAttachments(result.uploadedAttachments)
      const messageAttachments =
        uploadedMessageAttachments.length > 0 ? uploadedMessageAttachments : localAttachments
