...
const WorkflowContent = React.memo(
  ({
    ...
    workspaceId,
    reviewMode = false,
  }: WorkflowContentProps & { reviewMode?: boolean }) => {
    // State
    ...
    const handleAutoLayout = useCallback(async () => {
      ...
      try {
        ...
        const { applyAutoLayoutAndUpdateStore } = await import('@/app/workspace/[workspaceId]/w/[workflowId]/utils/auto-layout')
    ...
    useEffect(() => {
      ...
      const currentId = workflowId as string
      ...
      setIsWorkflowReady(false)
    }, [activeWorkflowId, workflowId, workflows, isLoading, reviewMode])

    // Init workflow
    useEffect(() => {
      if (reviewMode) return // <-- Prevent navigation/redirects in review mode
...
export default function WorkflowReviewModal({ workflowId, open, workspaceId,onClose }: { workflowId: string | null, open: boolean,workspaceId:string, onClose: () => void }) {
  const {
    loadWorkflowForReview
  } = useWorkflowRegistry()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  React.useEffect(() => {
    console.log(workflowId,open)
    if (!open || !workflowId) return
    setLoading(true)
    setError(null)
    loadWorkflowForReview(workflowId)
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
      <div className="bg-white rounded shadow-lg w-[90vw] h-[90vh] max-w-6xl max-h-[90vh] p-0 relative flex flex-col">
