  ...
  const fetchChatConfig = async () => {
    try {
      ...
      if (data.inputFormat && Array.isArray(data.inputFormat) && data.inputFormat.length > 0) {
        ...
        const fixedOnly = getFixedInputSetFields(data.inputFormat)
        const showModal =
  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      const payload: any = {
        ...
        // Include startBlockInputs when we have custom fields or fixed-input-set (API needs values for hasStartBlockInputValues)
        startBlockInputs:
