...
/**
 * Root-layout env script. Keep this helper here so `app/layout.tsx` can match
 * upstream's hosted/self-hosted split without inlining Arena's GHCR exception.
 *
 * Static {@link PublicEnvScript} is only safe when public env is fixed per image
 * (sim.ai hosted). Arena agent hosts share one GHCR image across envs and must
 * inject `NEXT_PUBLIC_*` at request time — including `NEXT_PUBLIC_ARENA_FRONTEND_APP_URL`
 * for the sidebar Back to Arena link.
 */
export function AppPublicEnvScript() {
  const appUrl = getEnv('NEXT_PUBLIC_APP_URL')
  const useStaticHostedEnv =
    appUrl === 'https://www.sim.ai' || appUrl === 'https://www.staging.sim.ai'
  return useStaticHostedEnv ? <PublicEnvScript /> : <RuntimePublicEnvScript />
}
