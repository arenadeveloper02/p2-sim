...
describe('settings navigation boundaries', () => {
  it('keeps Custom Blocks out of the Arena settings catalog', () => {
    expect(buildUnifiedSettingsCatalog().find(({ id }) => id === 'custom-blocks')).toBeUndefined()
    expect(WORKSPACE_SETTINGS_ITEMS.find(({ id }) => id === 'custom-blocks')).toBeUndefined()
  })
  ...
  it('offers hosted-only workspace sections on self-hosted only through their override', () => {
    ...
    expect(navigate(SELF_HOSTED)).not.toContain('inbox')
    expect(navigate({ ...HOSTED, features: inboxDisabled.features })).not.toContain('inbox')

    expect(navigate(SELF_HOSTED)).not.toContain('byok')
    expect(navigate(HOSTED)).not.toContain('byok')
  })
