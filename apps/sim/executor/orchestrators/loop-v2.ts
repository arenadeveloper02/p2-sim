...
import type { EdgeManagerV2 } from '@/executor/execution/edge-manager-v2'
...
const logger = createLogger('LoopOrchestratorV2')
...
export class LoopOrchestratorV2 {
  ...
  private resetNestedLoopScopes(loopId: string, ctx: ExecutionContext): void {
    ...
    for (const nodeId of loopConfig.nodes) {
      if (this.dag.loopConfigs.has(nodeId)) {
        const innerScope = ctx.loopExecutions?.get(nodeId)
        if (innerScope && innerScope.currentIterationOutputs.size > 0) {
          logger.warn(
            'Skipping nested loop scope reset while inner loop has in-flight iteration outputs',
            { parentLoopId: loopId, nestedLoopId: nodeId }
          )
          continue
        }
  ...
  restoreLoopEdges(loopId: string): void {
    ...
    if (this.edgeManager) {
      ...
      this.edgeManager.clearInternalDeactivatedEdges(allLoopNodeIds)
      this.clearAncestorLoopSentinelActivation(loopId)
    }
    ...
    for (const nodeId of allLoopNodeIds) {
      ...
      // Strip internal incoming edges only; preserve external entry edges (trigger, upstream blocks).
      for (const incomingSourceId of [...nodeToRestore.incomingEdges]) {
        if (allLoopNodeIds.has(incomingSourceId)) {
          nodeToRestore.incomingEdges.delete(incomingSourceId)
        }
      }
  ...
  /**
   * Clears stale activation flags on ancestor loop sentinel-ends so a nested loop
   * iteration reset cannot cause the parent loop to advance prematurely.
   */
  private clearAncestorLoopSentinelActivation(loopId: string): void {
    if (!this.edgeManager) return

    const sentinelIds = new Set<string>()
    for (const [candidateLoopId, config] of this.dag.loopConfigs) {
      if (candidateLoopId === loopId) continue
      if (this.isLoopNestedInside(loopId, candidateLoopId)) {
        sentinelIds.add(buildSentinelEndId(candidateLoopId))
      }
    }

    if (sentinelIds.size > 0) {
      this.edgeManager.clearNodesWithActivatedEdge(sentinelIds)
    }
  }
  ...
  private isLoopNestedInside(innerLoopId: string, ancestorLoopId: string): boolean {
    ...
    if (ancestorConfig.nodes.includes(innerLoopId)) return true
