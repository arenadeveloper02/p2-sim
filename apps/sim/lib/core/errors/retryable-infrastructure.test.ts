...
describe('isRetryableInfrastructureError', () => {
  ...
  it('recognizes driver messages that omit a structured connection code', () => {
    expect(isRetryableInfrastructureError(new Error('connection terminated unexpectedly'))).toBe(
      true
    )
    expect(
      isRetryableInfrastructureError(new Error('server closed the connection unexpectedly'))
    ).toBe(true)
  })
})
