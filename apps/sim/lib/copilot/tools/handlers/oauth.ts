...
export async function executeOAuthGetAuthLink(
  ...
  try {
    ...
    if (result.providerId === 'unipile_linkedin') {
      ...
      oauthUrl = authorizeUrl.toString()
    }
    ...
    return {
      ...
      output: {
        ...
        oauth_url: oauthUrl,
        instructions: `Open this URL in your browser to ${action} ${result.serviceName}: ${oauthUrl}`,
        provider: result.serviceName,
        providerId: result.providerId,
      },
    }
