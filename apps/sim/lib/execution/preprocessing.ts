...
export interface PreprocessExecutionOptions {
  ...
  /** Reuse a fork-resolved actor without repeating workspace payer lookup. */
...
export async function preprocessExecution(
  ...
  try {
    if (providedBillingAttribution) {
      ...
      logger.info(`[${requestId}] Reusing serialized billing attribution`, {
        billingActorUserId: validatedAttribution.actorUserId,
        billingEntity: validatedAttribution.billingEntity,
      })
    }
  ...
  if (!billingAttribution) {
    logger.error(`[${requestId}] Billing attribution resolution returned no snapshot`, {
      workflowId,
      workspaceId,
      actorUserId,
    })
  ...
  const resolvedBillingAttribution = billingAttribution
  ...
  const [banFailure, actorSubscription] = await Promise.all([banCheck, subscriptionFetch])
  ...
  const usageCheckTask = (async (): Promise<{
    ...
    try {
      const usageCheck = await checkServerSideUsageLimits(actorUserId, actorSubscription)
  ...
  if (!skipUsageLimits && !skipConcurrencyReservation && usageSnapshot) {
    try {
      const reservation = await reserveExecutionSlot({
        billingEntity: resolvedBillingAttribution.billingEntity,
        reservationId,
        plan: resolvedBillingAttribution.payerSubscription?.plan,
        enterpriseConcurrencyLimit:
          resolvedBillingAttribution.payerSubscription?.enterpriseConcurrencyLimit,
        currentUsage: usageSnapshot.currentUsage,
        limit: usageSnapshot.limit,
        ...(resolvedBillingAttribution.organizationId && usageSnapshot.memberUsage
          ? {
              member: {
                organizationId: resolvedBillingAttribution.organizationId,
                actorUserId,
  ...
  const plan = resolvedBillingAttribution.payerSubscription?.plan as SubscriptionPlan | undefined
  return {
    success: true,
    actorUserId,
    ...
    workflowRecord,
    actorSubscription,
    billingAttribution: resolvedBillingAttribution,
    executionTimeout: {
