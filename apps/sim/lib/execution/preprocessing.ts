...
import { getScheduleExecutionActorUserId } from '@/lib/workspaces/utils'
...
export interface PreprocessExecutionSuccess {
  ...
  actorUserId: string
  /**
   * Resolved actor identity for logging/lineage (client executions, personal
   * API keys, schedules). Optional because callers that predate actor lineage
   * still consume the result; present whenever actor resolution ran.
   */
...
export async function preprocessExecution(
  ...
  try {
    // Serialized boundary (background/resume): reuse the captured snapshot so
    // payer ownership can't change while work is queued or paused. Highest
    // precedence — a provided attribution overrides all other actor resolution.
    ...
    if (!actorUserId && triggerType === 'schedule') {
