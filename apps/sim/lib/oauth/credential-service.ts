...
export async function getOAuthToken(userId: string, providerId: string): Promise<string | null> {
  ...
  if (connections.length > 0) {
    ...
    const manualConnections = await db
      .select({
        ...
        providerAccountId: sql<string | null>`NULL`.as('provider_account_id'),
      })
      .from(accountTokens)
  ...
  if (accessTokenNeedsRefresh || instagramNeedsProactiveRefresh) {
    logger.info(`Refreshing OAuth token for user ${userId}, provider ${providerId}`)
...
export async function refreshTokenIfNeeded(
  ...
  // `account_tokens` only ever holds shared HubSpot portal tokens, so no other
  // provider can live outside `account` — skip the probe for them.
  let targetTable: 'account' | 'account_tokens' = 'account'
  if (credential.providerId === 'hubspot') {
    const [inAccountTokens] = await db
      .select({ id: accountTokens.id })
      .from(accountTokens)
      .where(eq(accountTokens.id, credential.id))
      .limit(1)
    if (inAccountTokens) targetTable = 'account_tokens'
  }
