...
- This app is embedded in a cross-origin iframe. Keep \`Content-Security-Policy: frame-ancestors *\` (set in proxy.ts) and never add \`X-Frame-Options: DENY\` / \`SAMEORIGIN\`.
...
- If \`emailId\` is missing or empty, proxy.ts rewrites to \`/access-denied\` — keep \`app/access-denied/page.tsx\` as a polished Arena DS UI that shows the message "Do not have access" (never replace it with plain text).
...
- Keep \`lib/arena-email-constants.ts\`, \`lib/arena-email.ts\`, \`components/arena-email-provider.tsx\`, \`app/access-denied/page.tsx\`, and root \`proxy.ts\` (Next 16; do not emit \`middleware.ts\`). Root \`app/layout.tsx\` must wrap children with \`ArenaEmailProvider\` using \`getArenaEmailId()\`.
