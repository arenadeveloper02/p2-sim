...
- If \`emailId\` is missing or empty, the app MUST respond with exactly: Do not have access
...
- Keep \`lib/arena-email-constants.ts\`, \`lib/arena-email.ts\`, \`components/arena-email-provider.tsx\`, and root middleware. Root \`app/layout.tsx\` must wrap children with \`ArenaEmailProvider\` using \`getArenaEmailId()\`.
...
- Never remove or bypass the Arena email gate, provider, or iframe headers.`
