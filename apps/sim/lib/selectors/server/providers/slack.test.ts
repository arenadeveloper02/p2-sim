...
function args(
  ...
  authentication: 'bot' | 'oauth' | 'user' | 'service_account' = 'bot',
  signal?: AbortSignal
  ...
  const oauthAccess = {
    ...
    credentialType:
      authentication === 'service_account' ? ('service_account' as const) : ('oauth' as const),
  }
  return {
    selectorKey,
    context: {
      oauthCredential: 'credential-1',
      ...(authentication === 'user' || authentication === 'service_account'
...
function requestedAuth(call: number): string | undefined {
  const init = mockFetchProviderJson.mock.calls[call]?.[1] as
...
describe('Slack server selector adapters', () => {
  ...
  it('lists Custom Bot OAuth with the user token and membership, skipping installer scoping', async () => {
    queueScopedAccount()
    mockFetchProviderJson.mockResolvedValueOnce(
      slackPage({
        channels: [
          channel('C001', 'general', false, true),
          channel('C002', 'unjoined'),
          channel('G001', 'private', true, true),
          channel('G002', 'private-unjoined', true),
        ],
      })
    )

    await expect(execute('slack.channels', { kind: 'list' }, 'user')).resolves.toEqual({
      kind: 'list',
      items: [
        { id: 'C001', label: '#general' },
        { id: 'G001', label: '#private' },
      ],
    })

    expect(mockGetCredential).toHaveBeenCalledOnce()
    expect(mockResolveSelectorOAuthAccessToken).not.toHaveBeenCalled()
    expect(requestedAuth(0)).toBe('Bearer xoxp-user-token')
    expect(requestedUrl(0).pathname).toBe('/api/conversations.list')
    expect(requestedUrl(0).searchParams.get('types')).toBe('public_channel,private_channel')
    expect(mockFetchProviderJson).toHaveBeenCalledOnce()
  })

  it('falls back to public channels for Custom Bot user-token listing', async () => {
    mockFetchProviderJson
      .mockRejectedValueOnce(new Error('private scope unavailable'))
      .mockResolvedValueOnce(
        slackPage({ channels: [channel('C001', 'general', false, true)] }, 'public-page-2')
      )

    const result = await execute('slack.channels', { kind: 'list' }, 'user')
    ...
    if (result.kind !== 'list' || !result.nextCursor) {
      throw new Error('Expected a public-only user cursor')
    }
    queueScopedAccount()
    await expect(
      execute('slack.channels', { kind: 'list', cursor: result.nextCursor }, 'oauth')
    ).rejects.toMatchObject({ name: 'SelectorContextUnavailableError' })
  })

  it('fails closed when Custom Bot OAuth has no user token', async () => {
    mockGetCredential.mockResolvedValueOnce({ idToken: null })
    await expect(execute('slack.channels', { kind: 'list' }, 'user')).rejects.toEqual(
  ...
  it('uses the bot token for Custom Bot service accounts', async () => {
    mockFetchProviderJson.mockResolvedValueOnce(
      slackPage({ channels: [channel('C001', 'general', false, true)] })
    )
    await expect(execute('slack.channels', { kind: 'list' }, 'service_account')).resolves.toEqual({
      kind: 'list',
      items: [{ id: 'C001', label: '#general' }],
    })
    expect(mockGetCredential).not.toHaveBeenCalled()
    expect(mockResolveSelectorOAuthAccessToken).toHaveBeenCalledOnce()
    expect(requestedAuth(0)).toBe('Bearer xoxb-server-only-token')
  })

  it('hydrates saved users with the Custom Bot user token', async () => {
    mockFetchProviderJson.mockResolvedValueOnce(
      slackPage({ user: user('U999', 'saved', 'Saved User') })
    )
    await expect(execute('slack.users', { kind: 'detail', id: 'U999' }, 'user')).resolves.toEqual({
      kind: 'detail',
      item: { id: 'U999', label: 'Saved User' },
    })
    expect(requestedAuth(0)).toBe('Bearer xoxp-user-token')
  ...
  it('keeps pasted bot tokens on the bot path even when Custom Bot is selected', async () => {
    mockFetchProviderJson.mockResolvedValueOnce(
      slackPage({ channels: [channel('C001', 'general')] })
    )
    const input = args('slack.channels')
    input.context.authMethod = 'bot_token'
    await expect(slackSelectorAttachments['slack.channels'].execute(input)).resolves.toEqual({
      kind: 'list',
      items: [{ id: 'C001', label: '#general' }],
    })
    expect(mockGetCredential).not.toHaveBeenCalled()
