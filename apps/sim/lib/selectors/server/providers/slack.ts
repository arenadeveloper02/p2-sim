...
const logger = createLogger('SlackSelector')
...
type SlackCursorState =
  ...
  | { mode: 'oauth' | 'bot-all' | 'bot-public' | 'user-all' | 'user-public'; conversations: string }
...
type SlackUnscopedCursorMode = 'oauth' | 'bot-all' | 'bot-public' | 'user-all' | 'user-public'

/**
 * Custom Bot on a connected Slack OAuth account (`authMethod === 'bot_token'`)
 * uses the user token (`idToken` / `xoxp-`). Sim Bot, pasted `xoxb-` tokens,
 * and reusable custom-bot service accounts use the bot token.
 */
async function resolveSlackSelectorAuthentication(
  args: ExecuteServerSelectorArgs
): Promise<SlackChannelAuthentication> {
  if (!args.credential) throw new SelectorConnectionUnavailableError()
  const preferUserToken =
    args.context.authMethod === 'bot_token' &&
    !args.credential.fixedToken &&
    args.credential.access?.credentialType === 'oauth'
  ...
  if (preferUserToken) {
    ...
    if (!userToken) {
      logger.warn('Custom Bot selector requested a user token but idToken is missing')
      throw new SelectorConnectionUnavailableError(401)
    }
    args.protectedValues.add(userToken)
    return {
      accessToken: userToken,
      tokenKind: 'user',
      isBotCredential: false,
      scopedUserId: null,
    }
  }
  ...
  return {
    ...
    tokenKind: 'bot',
    isBotCredential,
    scopedUserId: isBotCredential ? null : await readScopedSlackUserId(args),
  }
}

function unscopedCursorMode(
  authentication: SlackChannelAuthentication,
  cursor: Extract<SlackCursorState, { conversations: string }> | undefined
): SlackUnscopedCursorMode {
  if (authentication.tokenKind === 'user') {
    return cursor?.mode === 'user-public' ? 'user-public' : 'user-all'
  }
  if (authentication.isBotCredential) {
    return cursor?.mode === 'bot-public' ? 'bot-public' : 'bot-all'
  }
  return 'oauth'
}

function isPublicOnlyCursorMode(mode: SlackUnscopedCursorMode): boolean {
  return mode === 'bot-public' || mode === 'user-public'
}

function publicFallbackCursorMode(
  mode: SlackUnscopedCursorMode
): 'bot-public' | 'user-public' | null {
  if (mode === 'bot-all') return 'bot-public'
  if (mode === 'user-all') return 'user-public'
  return null
}

function includeListedChannel(
  channel: SlackChannel,
  authentication: SlackChannelAuthentication
): boolean {
  if (authentication.tokenKind === 'user') return Boolean(channel.is_member)
  return !(channel.is_private && !channel.is_member)
}
...
function assertChannelCursorMode(
  ...
  if (authentication.tokenKind === 'user') {
    if (cursor.mode !== 'user-all' && cursor.mode !== 'user-public') {
      throw new SelectorContextUnavailableError()
    }
    return
  }
...
async function listUnscopedSlackChannels(
  ...
  let mode = unscopedCursorMode(authentication, cursor)
  ...
  try {
    page = await fetchChannelPage(
      ...
      authentication.accessToken,
      {
        types: isPublicOnlyCursorMode(mode) ? 'public_channel' : 'public_channel,private_channel',
        exclude_archived: 'true',
      },
      cursor?.conversations
    )
    ...
    if (args.signal?.aborted) throw error
    const fallbackMode = publicFallbackCursorMode(mode)
  ...
  const items = page.channels.flatMap((channel) => {
    if (!includeListedChannel(channel, authentication)) return []
    const option = channelOption(channel)
    return option ? [option] : []
  })
  return listSelectorResult(
...
async function hydrateSlackChannel(
  ...
  if (authentication.tokenKind === 'user' && !data.channel.is_member) {
    return detailSelectorResult(null)
  }
...
async function executeSlackChannels(args: ExecuteServerSelectorArgs) {
  ...
  const authentication = await resolveSlackSelectorAuthentication(args)
  ...
  if (authentication.tokenKind === 'bot' && authentication.scopedUserId) {
  ...
  return listUnscopedSlackChannels(
    args,
    authentication,
    cursor as Extract<SlackCursorState, { conversations: string }> | undefined
  )
}
...
async function executeSlackUsers(args: ExecuteServerSelectorArgs) {
  ...
  const { accessToken } = await resolveSlackSelectorAuthentication(args)
  if (args.request.kind === 'detail') {
