...
export function buildResourceAttachments(
  ...
  const attachments = resources.flatMap<ResourceAttachment>((resource) => {
    ...
    // A log panel stays open as a UI tab but is never sent as chat context;
    // execution logs reach the model through the log tools instead.
    if (resource.type === 'log') return []
