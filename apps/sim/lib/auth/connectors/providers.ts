...
/** Tenant-aware Microsoft Entra authorize/token URLs (Arena / self-hosted). */
...
export function buildConnectorProviders(): GenericOAuthConfig[] {
  const providers: GenericOAuthConfig[] = [
...

    {
      ...
      // Arena: exchange via internal proxy so bot + user tokens are both preserved
      // (user token returned as id_token). Direct slack.com oauth.v2.access drops that.
