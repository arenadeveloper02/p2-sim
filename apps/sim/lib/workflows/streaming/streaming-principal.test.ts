...
describe('selected output principal', () => {
   ...
   * output is materialized from the final result instead. Every path read must
   * run as the principal behind the run, or a member-only knowledge-base file
   * in the output is read as nobody on the final-frame path. Raw-then-parsed
   * resolution can navigate more than once per extract.
   */
  it('reads a selected output as the executing principal on the chunk and final paths', async () => {
    ...
    expect(mockNavigatePathAsync.mock.calls.length).toBeGreaterThanOrEqual(2)
