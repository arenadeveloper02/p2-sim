...
export const deployWorkflowChat = defineAuthorizedWorkflowUseCase({
  ...
  async execute({ principal, input, context }) {
    ...
    const deploymentType =
      ...
      (existingDeployment?.deploymentType as ChatDeploymentType | undefined) ??
      'chat'
    const redirectUrl = input.redirectUrl ?? existingDeployment?.redirectUrl ?? undefined
    const department = input.department ?? existingDeployment?.department ?? undefined
