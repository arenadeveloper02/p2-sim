...
export async function performChatDeploy(
  ...
  const { workflowId, userId, identifier, title, password, deploymentType = 'chat' } = params
  ...
  if (existingDeployment) {
    ...
    logger.info('Updated existing chat deployment', {
      chatId,
      workflowId,
      identifier,
      authType,
    ...
    logger.info('Created new chat deployment', {
      chatId,
      workflowId,
      userId,
      identifier,
