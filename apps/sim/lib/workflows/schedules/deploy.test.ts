...
describe('Schedule Deploy Utilities', () => {
  ...
  describe('createSchedulesForDeploy', () => {
    ...
    it('should update the existing row instead of inserting for existing schedules', async () => {
      const blocks: Record<string, BlockState> = {
        'block-1': {
          id: 'block-1',
          type: 'schedule',
      ...
      queueTableRows(schemaMock.workflowSchedule, [
        { id: 'existing-schedule-id', blockId: 'block-1' },
      ])
    ...
    it('should collapse duplicate rows for a block onto the first row', async () => {
      const blocks: Record<string, BlockState> = {
        'block-1': {
          id: 'block-1',
          type: 'schedule',
      ...
      queueTableRows(schemaMock.workflowSchedule, [
        { id: 'duplicate-1', blockId: 'block-1' },
        { id: 'duplicate-2', blockId: 'block-1' },
      ])

      const result = await createSchedulesForDeploy('workflow-1', blocks)

      expect(result.success).toBe(true)
      expect(result.scheduleId).toBe('duplicate-1')
