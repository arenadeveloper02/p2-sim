  ...
  semrush: [
    {
      from: 'trackingUrl',
      to: 'url',
      whenOperation: ['semrush_organic_positions'],
    },
  ],
}

/**
 * Rewrites a stored `operation` dropdown value onto the current option id.
 * Used when a block's operation ids were renamed (Semrush split a generic
 * query into dedicated tools) so the card still selects a live option.
 */
export const SUBBLOCK_OPERATION_VALUE_MIGRATIONS: Record<string, Record<string, string>> = {
  semrush: {
...
/**
 * Rewrites a stored operation dropdown onto the current option id when the
 * block's operation ids were renamed. Must run before ID migrations so
 * operation-scoped renames see the live option.
 */
function migrateBlockOperationValue(
  blockType: string,
  subBlocks: Record<string, BlockState['subBlocks'][string]>
): { subBlocks: Record<string, BlockState['subBlocks'][string]>; migrated: boolean } {
  const mapping = SUBBLOCK_OPERATION_VALUE_MIGRATIONS[blockType]
  if (!mapping) return { subBlocks, migrated: false }

  const current = selectedOperation(subBlocks)
  if (current === null || !Object.hasOwn(mapping, current)) {
    return { subBlocks, migrated: false }
  }

  const next = mapping[current]
  const existing = subBlocks.operation
  if (isPlainRecord(existing)) {
    return {
      subBlocks: { ...subBlocks, operation: { ...existing, value: next } },
      migrated: true,
    }
  }
  return {
