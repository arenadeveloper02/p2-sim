...
async function readFileContent(filePath: string): Promise<string> {
  try {
    ...
    if (fileExtension === '.pdf') {
      ...
      console.log(`PDF parsed successfully: ${result.metadata?.pageCount || 0} pages, ${result.content.length} characters`);
...
export async function generateFigmaDesign(
  inputs: FigmaDesignInputs
): Promise<FigmaDesignResult> {
  try {
...
`;
  ...
  if (inputs.brandGuidelinesFile) {
    ...
    try {
      ...
      console.warn(
        "Could not read brand guidelines file:",
        inputs.brandGuidelinesFile
      );
    }
  }
  ...
  if (inputs.additionalDataFile) {
    ...
    try {
      ...
      console.warn(
        "Could not read additional data file:",
        inputs.additionalDataFile
      );
    }
  }
 ...
 */
async function generateHTMLCSS(
  systemPrompt: string,
  userPrompt: string
): Promise<string> {
 ...
 */
async function logIframeInfo(driver: WebDriver, level: string = "main"): Promise<void> {
  try {
    ...
    for (let i = 0; i < iframes.length; i++) {
      try {
        ...
        console.log(`[${level}] Iframe ${i}: id="${id}", name="${name}", src="${src?.substring(0, 50)}..."`);
      } catch (e) {
 ...
 */
async function findDeepestIframeWithEditor(driver: WebDriver, maxDepth: number = 5): Promise<boolean> {
  try {
    const iframes = await driver.findElements(By.css("iframe"));
    ...
    if (iframes.length === 0) {
      ...
      const editorTab = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[1]/label[4]"));
      const specificSelector = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[2]/div[4]/section/div[1]"));
    ...
    for (let i = 0; i < iframes.length; i++) {
      try {
        ...
        if (editorTab.length > 0) {
          ...
          return true;
        } else if (specificSelector.length > 0) {
          console.log(`✓ Found specific selector in iframe ${i}`);
          return true;
        }
    ...
    console.log(`Error in findDeepestIframeWithEditor: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  }
}
...
async function switchToPluginIframe(driver: WebDriver): Promise<boolean> {
  ...
  for (const selector of innerIframeSelectors) {
    try {
      ...
      console.log(`⚠️ Selector ${selector.toString()} failed: ${e instanceof Error ? e.message : String(e)}`);
      // Continue to next selector
    }
  }
    ...
    if (nestedIframes.length > 0) {
      ...
      for (let i = 0; i < nestedIframes.length; i++) {
        try {
          ...
          // Check if this iframe contains the Editor tab or the specific selector
          const editorTab = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[1]/label[4]"));
          const specificSelector = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[2]/div[4]/section/div[1]"));
          ...
          if (editorTab.length > 0) {
            ...
            console.log("✓ Confirmed specific selector found in innermost iframe");
            return true;
          }
          ...
          console.log(`⚠️ Could not switch to nested iframe ${i}: ${nestedError instanceof Error ? nestedError.message : String(nestedError)}`);
          await driver.switchTo().parentFrame();
        }
      }
    }
  } catch (e) {
    console.log("⚠️ No additional nested iframes found or error checking: ", e instanceof Error ? e.message : String(e));
  }
  
  // Verify we're in the correct iframe by checking for Editor tab or specific selector
  try {
    const editorTab = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[1]/label[4]"));
    const specificSelector = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[2]/div[4]/section/div[1]"));
    ...
    if (editorTab.length > 0) {
      ...
      console.log("⚠️ Neither Editor tab nor specific selector found, but iframe structure is correct");
    ...
    try {
      ...
      for (let i = 0; i < iframes.length; i++) {
        try {
          ...
          const hasEditorTab = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[1]/label[4]")).then(elements => elements.length > 0);
          if (hasEditorTab) {
          ...
          try {
            ...
            for (let j = 0; j < nestedIframes.length; j++) {
              try {
                ...
                const hasNestedEditorTab = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[1]/label[4]")).then(elements => elements.length > 0);
                ...
                try {
                  ...
                  console.log(`Found ${deepIframes.length} deep nested iframe(s) in iframe ${i}-${j}`);
                  ...
                  for (let k = 0; k < deepIframes.length; k++) {
                    try {
                      ...
                      const hasDeepEditorTab = await driver.findElements(By.xpath("//*[@id='container-tabs']/div/div[1]/label[4]")).then(elements => elements.length > 0);
                      if (hasDeepEditorTab) {
  ...
  try {
    ...
    try {
      ...
      const createButton = await driver.wait(
        until.elementLocated(
          By.xpath("//button[contains(., 'Create') or contains(., 'New')]")
        ),
        10000
      );
      ...
      const designOption = await driver.wait(
        until.elementLocated(
          By.xpath("//span[contains(text(),'Design')]")
        ),
        5000
      );
    ...
    try {
      ...
      const pluginButton = await driver.wait(
        until.elementLocated(
          By.xpath("//*[@id='react-page']/div/div/div/div[1]/div[2]/div/div/div/div/div/div/div[1]/div/div/div[2]/div/div/div/button[2]")
        ),
        10000
      );
      ...
      console.log("Direct plugin button click failed, trying keyboard shortcut...", error instanceof Error ? error.message : String(error));
    ...
    try {
      ...
      try {
        ...
        const searchInput = await driver.wait(
          until.elementLocated(
            By.xpath("//*[@id='react-page']/div/div/div/div[1]/div[1]/div/div[1]/div[12]/div/div/div/div/div/div/div/div[1]/input")
          ),
          5000
        );
      ...
      try {
        const pluginOption = await driver.wait(
          until.elementLocated(
            By.xpath("//*[@id='react-page']/div/div/div/div[1]/div[1]/div/div[1]/div[12]/div/div/div/div/div/div/div/div[3]/div/div/div/div/div/div/button[1]")
          ),
          5000
        );
      ...
      try {
        ...
        try {
          const addToCanvasButton = await driver.wait(
            until.elementLocated(
              By.xpath("//button[contains(text(), 'Add to Canvas') or contains(text(), 'Add to canvas')]")
            ),
            10000
          );
          await addToCanvasButton.click();
          console.log("✓ Clicked Add to Canvas button");
          await driver.sleep(3000); // Wait for design to be added to canvas
        } catch (e) {
          console.log("Add to Canvas button not found, trying alternative selectors...");
          ...
          let addToCanvasClicked = false;
          for (const selector of addToCanvasSelectors) {
            try {
              const addToCanvasElement = await driver.findElement(By.xpath(selector));
              await addToCanvasElement.click();
              console.log(`✓ Clicked Add to Canvas using selector: ${selector}`);
              addToCanvasClicked = true;
              break;
            } catch (e) {
              // Continue to next selector
            }
          ...
          if (!addToCanvasClicked) {
            console.log("Could not find Add to Canvas button, trying JavaScript...");
            await driver.executeScript(`
              const buttons = document.querySelectorAll('button');
              for (let button of buttons) {
                if (button.textContent && button.textContent.toLowerCase().includes('add to canvas')) {
                  button.click();
                  console.log('Clicked Add to Canvas button via JavaScript');
                  break;
                }
              }
            `);
            console.log("✓ Attempted JavaScript click for Add to Canvas");
            await driver.sleep(2000);
          }
        }
      } catch (e) {
        ...
        console.log("Looking for Add to Canvas button (alternative path)...");
        try {
          const addToCanvasButton = await driver.wait(
            until.elementLocated(
              By.xpath("//button[contains(text(), 'Add to Canvas') or contains(text(), 'Add to canvas')]")
          ...
          await addToCanvasButton.click();
          console.log("✓ Clicked Add to Canvas button (alternative path)");
          await driver.sleep(3000); // Wait for design to be added to canvas
        } catch (e) {
          console.log("Add to Canvas button not found in alternative path, trying JavaScript...");
          await driver.executeScript(`
            const buttons = document.querySelectorAll('button');
            for (let button of buttons) {
              if (button.textContent && button.textContent.toLowerCase().includes('add to canvas')) {
                button.click();
                console.log('Clicked Add to Canvas button via JavaScript (alternative path)');
                break;
              }
            }
          `);
          console.log("✓ Attempted JavaScript click for Add to Canvas (alternative path)");
          await driver.sleep(2000);
        }
      }
    ...
    try {
      ...
      try {
        const closeButton = await driver.findElement(
          By.xpath("//button[contains(@aria-label, 'Close') or contains(@title, 'Close') or contains(text(), 'Close')]")
        );
...
export async function exampleUsage() {
  const result = await generateFigmaDesign({
    ...
    prompt: "Create a modern landing page for a SaaS product with a hero section, features section, and CTA buttons",
