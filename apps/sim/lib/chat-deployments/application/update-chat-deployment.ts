...
import { syncWorkflowGoldenQueriesFromCustomizations } from '@/lib/chat-deployments/golden-queries'
import {
...
type ChatDeploymentType = 'chat' | 'app'
...
export interface UpdateChatDeploymentInput {
  ...
  deploymentType?: ChatDeploymentType
  redirectUrl?: string
...
export const updateChatDeployment = defineAuthorizedWorkspaceUseCase({
  ...
  async execute({ principal, input, context }): Promise<UpdateChatDeploymentResult> {
    ...
    if (input.deploymentType === 'app') {
      const redirectUrl = input.redirectUrl ?? existing.redirectUrl
      if (!redirectUrl?.trim()) {
        throw new OrchestrationError(
          'validation',
    ...
    if (input.customizations) {
      const { goldenQueries: _goldenQueries, ...restCustomizations } = input.customizations
      values.customizations = restCustomizations
    }
    if (input.authType) {
    ...
    if (input.deploymentType) {
      values.deploymentType = input.deploymentType
      values.redirectUrl =
        input.deploymentType === 'app' ? (input.redirectUrl?.trim() ?? null) : null
    }
    ...
    await syncWorkflowGoldenQueriesFromCustomizations({
      workflowId: existing.workflowId,
      userId: actingUserId,
      goldenQueries: input.customizations?.goldenQueries,
    })
