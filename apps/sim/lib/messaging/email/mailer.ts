...
export async function sendEmail(options: EmailOptions): Promise<SendEmailResult> {
  try {
    ...
    if (activeProviders.length === 0) {
      logger.warn('Email not sent (no email provider active):', {
        to: data.to,
        subject: data.subject,
        from: data.senderEmail,
        hint: 'Configure RESEND_API_KEY, AWS_SES_REGION/AWS_REGION with AWS credentials, SMTP_*, or AZURE_ACS_CONNECTION_STRING',
      })
