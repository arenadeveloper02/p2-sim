  ...
  },
  {
    ...
    clauses: [
      ...
      },
      {
        ...
        prompt:
          'Format dates, numbers, currencies, and percentages in labels and literal copy. Bound ISO date and datetime values ({item.date}, createdAt) are formatted by the host — bind the field; do not dump the raw timestamp and do not invent a second formatted copy of API data. When the brief names a date format, set Card.dateFormat or `{item.date|DD/MM/YYYY}` (presets short, medium, long, iso, numeric, numeric-eu, datetime, relative, ago). relative/ago is calendar-stable for date-only values (today, yesterday, 3 days ago) and coarse for datetimes. Bound numbers stay raw unless the brief names a format: Stat/Card.numberFormat or `{item.price|currency}` / `{item.rate|percent}` (presets number, integer, currency, usd, eur, gbp, percent, compact). Table columns may add footer aggregates (`amount|sum`) or an index column (`#`). Numeric Stat state values are grouped by the host. Do not invent API values to make a Stat look filled.',
      },
    ],
  },
  {
