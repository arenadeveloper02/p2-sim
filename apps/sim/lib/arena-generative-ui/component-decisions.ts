...
export const ARENA_GENERATIVE_UI_COMPONENT_SELECTION_PROMPT = [
  ...
  'PageHeader — task-oriented pages with no single entity (search, generate, wizard step). Use instead of a bare Heading.',
  'Card — an independent conceptual group, or a Repeat entity item. Do not wrap every Section in a Card.',
  'Table — comparable rows of the same scalar fields, no per-row visual identity. Not for narrative or prose.',
  ...
  'Tabs — three or more peer top-level views. Not sequential workflow steps (those are pages with Next/Back).',
