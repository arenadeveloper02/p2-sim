...
export const ARENA_GENERATIVE_UI_COMPONENT_SELECTION_PROMPT = [
  ...
  'Card — an independent conceptual group, or a Repeat entity item. Not a FAQ/criteria row.',
  'Disclosure — a short title that reveals a longer body on click (FAQ, criteria, justification). Use inside Repeat. Title is {item.question} or a static label; body is DataText/KeyValue on item.*. The host paints the chevron and expand state. Do not emit Icon chevrons or duplicate open/closed copies. Do not use selectItem to expand a row. Not Drawer and not History Open.',
  'Table — comparable rows of the same scalar fields, no per-row visual identity. Header click sorts loaded rows. Column pipes: `date|relative`, `amount|sum`, `#`.',
  'Calendar — month or week plot of dated collection rows. Use when representation is calendar. Clicking a chip copies the row like Repeat selectItem. Undated rows go in Unscheduled.',
  'Repeat — each item has its own visual representation (Card, avatar, action, or Disclosure). Put Repeat inside Grid; never wrap Grid in Repeat. reorderable only when the brief asked to reorder dummy/local rows.',
  ...
  'Chip — same-page view switch (result panels on one Generator or History detail): one Chip per view, shared setValue field, distinct values, no actionId. Put a horizontal Chip row above the bound panels. A left vertical rail only when the brief explicitly asks for it. Do not duplicate brand/muted chips for selected state. Suggestion chips stay a single authored-tone Chip.',
  ...
  'Chart — trends, comparisons, or mix across a bound numeric series (bar, line, area, pie). statePath on an array of records plus categoryField and series field keys, like Table.columns. Not a compact under-Stat trend (that is Sparkline). Not a comparable row list (that is Table).',
  'Sparkline — compact number[] trend under a Stat or inside a KPI Card. Not a full chart: no axes, legends, or multiple series.',
  ...
  'Skeleton — only for a static-children region whose layout is known. Bound Table, Repeat, Stat, KeyValue, DataText, Chart, and Sparkline already skeleton; prefer statePath.',
  'KeyValue — one record’s scalar pairs. Not a collection (Table/Repeat), not a wrapper object that contains arrays, and not prose (DataText). Nested arrays on a bound object still render as a list or Table — do not dump JSON.',
