...
export const arenaGenerativeUiCatalog = defineCatalog(reactSchema, {
  components: {
    ...
    KeyValue: {
      ...
      description:
        'Two-column detail list of scalar pairs. items is newline-separated "key: value" rows, or set statePath to a host-state object. Arrays on that object are not JSON — string[] is a list, object[] is a Table. Do not bind KeyValue to a collection hostKey or to a wrapper that only exists to hold arrays (coverage_report with citations_found[]) — those are Repeat or Table. A bound list with no entries shows emptyText (default "No details").',
    },
    Alert: {
      props: z.object({
        text: z.string(),
...
export const ARENA_GENERATIVE_UI_ACTION_RESULT_RULE = [
  'CTA results: each binding includes layoutPlan — bind only those hostKeys as statePath. Nested collections lift to the last segment ("history"); do not bind run_data.history, data.articles, output.articles, or response.articles. Wrapper objects that contain arrays (coverage_report with citations_found[]) are not KeyValue — bind each collection hostKey as Table or Repeat, and wrapper scalars with dotted hostKeys (coverage_report.summary). A response that is an array or a plain value lands under "result". A markdown string field (article_data, or a typo like artical_data) lands at that key and is copied to "content" as the prose body — not a JSON dump of the object. Bind DataText to "content" or the string field name; never "field.content" unless outputSchema shows an object with a content child.',
  ...
  'When a binding declares outputSchema, bind layoutPlan.hostKeys as statePath — never raw outputSchema names, never result[].…, and never an output. / data. / response. prefix (coverage_report.summary, not output.coverage_report.summary). An array hostKey such as "articles" with children "articles[].title" becomes Table statePath="articles" with columns from those child names, or Repeat inside a Grid when each item needs its own Card, link, or action; a string[] such as faq_questions_added[] is Repeat of Text; a single number or boolean becomes Stat; a scalar object becomes KeyValue of those scalars only; a markdown string becomes DataText on that name or "content", never "field.content"; only fall back to DataText statePath="content" for unstructured prose or when the binding declares no outputSchema. showWhen on the results chrome must use that same key as the DataText (content or the string field), not one key for the toolbar and another for the article. Copy Markdown and Download PDF are host actions: Button copyContent true / downloadPdf true (or those labels) with no actionId — the host copies or downloads the visible DataText. Never bind them to the generate API.',
