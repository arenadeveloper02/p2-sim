...
export const ARENA_GENERATIVE_UI_ACTION_RESULT_RULE = [
  'CTA results: when an action succeeds the host merges the response object top-level keys into app state, so a statePath is the response key itself — use "articles" or "history", never "data.articles", "output.articles", or "response.articles". Nested collections such as run_data.history are also copied to "history". A response that is an array or a plain value lands under "result". "content" always holds a text rendering of the whole response.',
  ...
  'When list items already include a prose field (history[].output, items[].content), Open is Button selectItem true with no actionId; the host copies that field to content. Do not invent a second fetch for a field already on the row. A separate results page binds DataText to content. A same-page History Open hides the list with showWhen "!selectedId" and shows the markdown with showWhen "selectedId" plus Back clearItem true.',
].join(' ')
