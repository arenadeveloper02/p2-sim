...
export const ARENA_GENERATIVE_UI_DESIGN_GUIDELINES = [
  ...
  'Every generate reply includes the default theme. Page → Section (width wide, no maxWidth) → PageHeader, then groups of Grid / Columns / Card with gap "24px". Surfaces are exactly two — the page canvas and the Card/Stat surface — both supplied by the host. Content avatars and company logos are allowed; do not add an app wordmark.',
].join('\n')
