...
export const ARENA_GENERATIVE_UI_ANTI_PATTERNS_PROMPT = [
  ...
  'table — Never use Table for narrative entities. That is Repeat of Cards. Table is comparable scalars with no per-row identity. Never invent columns that are not on the row or a closed host token (`#`, `amount|sum`, `date|relative`).',
