...
describe('parseArenaGenerativeRepresentation', () => {
  it('accepts the closed enum and aliases, and fails unknown values open to auto', () => {
    ...
    expect(parseArenaGenerativeRepresentation('kanban_board')).toBe('kanban')
    expect(parseArenaGenerativeRepresentation('chronology')).toBe('timeline')
...
describe('ARENA_GENERATIVE_UI_REPRESENTATION_PROMPT', () => {
  it('separates representation from archetype and forbids invented types', () => {
    ...
    expect(ARENA_GENERATIVE_UI_REPRESENTATION_PROMPT).toContain('Use Timeline')
    expect(ARENA_GENERATIVE_UI_REPRESENTATION_PROMPT).toContain('Use Kanban')
    expect(ARENA_GENERATIVE_UI_REPRESENTATION_PROMPT).not.toContain('No Timeline type')
