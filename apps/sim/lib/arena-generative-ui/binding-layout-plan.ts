...
export function resultLayoutFromPlan(plan: BindingLayoutPlan): string {
  ...
  if (proseCollection) {
    ...
    return `list items include a prose field — Repeat cards bind only short scalars with ${sample}; Open is Button selectItem true (no actionId) and copies prose to content plus named result fields (enhanced_article) onto the same host keys as generate, not inputs; History-style same-page Open (no Workspace or Drawer inspector) hides the list with showWhen "!selectedId" and shows Chip setValue result views plus markdown with showWhen "selectedId" plus clearItem Back; Workspace and Drawer keep the collection visible — do not hide navigator or primary with !selectedId; do not bind item.${prose} inside Repeat; Results after Generate echo form names ({targetKeyword}), not history keys ({keyword})`
  }
