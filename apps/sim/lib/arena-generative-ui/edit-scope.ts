...
const SCOPE_SYSTEM_PROMPT = [
  ...
  'Operators name the thing they see — "the search box", "the score", "Open on a row" — more often than a page path. Map those to pages from the index (titles, components, onLoad, navigatesTo). "the form" / search / generate is usually home; "the answer" / output / score / results table is usually the destination of that submit; a row, Open, or history is the list page. Binding a field to result "tabs" (Enhance Article, Coverage, …) after View plus the same keys after form submit mutates both History and the generate page — include both. Include every page that change actually mutates (a submit-label tweak is home only; "show X on results" is results; moving waiting chrome off the form is both).',
