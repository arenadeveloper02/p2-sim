...
export const ARENA_GENERATIVE_UI_COMPONENT_RULES = [
  ...
  'Navigation: Tabs items are newline-separated "Label|path" lines with distinct page paths for two or more peer destinations (Generator|home, History|history); set activePath to the current path. Same-page result views (Enhanced Article / Gap Analysis on one Generator page) are Chip setValue with one Chip per view — put that row above the bound panels (or left of a horizontal split); the host paints the selected Chip and does not drop History selectedId. Do not emit two Chips per label. A search hero omits Tabs. Detail and progress pages are reached with NavLink/navigateTo and offer a Back NavLink.',
