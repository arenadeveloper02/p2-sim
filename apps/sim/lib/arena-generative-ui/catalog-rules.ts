...
export const ARENA_GENERATIVE_UI_COMPONENT_RULES = [
  ...
  'Chrome: start a page with PageHeader (kicker, title, subtitle, optional trailing child) instead of a bare Heading. Filter children are Select, TextInput, DateInput, or Chip named after collection columns — the host filters locally when those fields have no actionId. Modal and Drawer use showWhen; open with a Button setValue that sets the flag (`creating=true`); close with a ghost Button setValue that clears it (`creating=`), or clearItem when showWhen is selectedId.',
