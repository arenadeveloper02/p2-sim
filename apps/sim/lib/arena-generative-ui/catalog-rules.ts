...
export const ARENA_GENERATIVE_UI_COMPONENT_RULES = [
  ...
  'Card identity: Card.title is a short name, keyword, or hostname — never a full http(s) URL. Put the URL in subtitle, description, or a Link. History run cards bind item.keyword or item.name (plus item.client); category/score are Badge children; date is footerText.',
  'Collections: put a Repeat inside a Grid (columns 2 or 3) or Stack, bound to the array statePath; Repeat\'s children are the per-item template and render once per element. Honour a brief that asks for two cards per row with Grid columns 2 wrapping Repeat — do not stack History run cards in a single column. Never unroll a live array into one static Card per item, and never wrap Grid in Repeat (that produces N grids). Bind per-item fields with statePath "item.field". Put per-item values into navigation and hrefs with "{item.id}" — NavLink.to "order?id={item.id}" opens the detail page so its onLoad receives that id. A Button.selectItem inside Repeat copies the row into host state without an API call; a Button.actionId sends the item fields as the action input. Never bind a long prose field (output, content, body) on Card or as always-visible Repeat copy — not item.output, not Card.description, not a Table column. FAQ, criteria, and justification arrays are Repeat of Disclosure: title "{item.question}" (or a static label), body DataText/KeyValue on item.* — the host paints the chevron. Do not use selectItem to expand a row. Bound ISO dates on cards ({item.date}, createdAt) are formatted by the host — bind the field; do not dump the raw timestamp. When the brief names a date format, set Card.dateFormat or bind `{item.date|DD/MM/YYYY}` or `{item.date|relative}` (presets: short, medium, long, iso, numeric, numeric-eu, datetime, relative, ago; tokens: YYYY MM DD MMM MMMM h:mm A). Bound numbers stay raw unless the brief names a format: Card/Stat.numberFormat or `{item.price|currency}` / `{item.rate|percent}` / `{item.count|integer}` (presets number, integer, currency, usd, eur, gbp, percent, compact). Table columns may use `#` (visible index), `date|relative`, and footer aggregates `amount|sum` (sum, avg, min, max, count). Do not invent API fields such as sentiment. Emit Table on a markdown/JSON/CSV string only when the brief asked to show that field as a table. Dated collections that should plot on a month/week grid use Calendar, not a dated Repeat. Set Table/Repeat reorderable only when the brief asked to reorder dummy/local rows. Do not invent a second formatted copy of API data.',
  'Loaded row selection: when list items already include a prose field (history[].output, items[].content), Open is Button selectItem true with no actionId. It copies prose to content plus selected/selectedId, and named result fields (enhanced_article, coverage_report) onto the same host keys as generate — it does not restamp inputs. If the brief opens a separate results/detail page, add navigateTo that page (no onLoad there) so DataText statePath "content" or the string field name shows the row. Same-page History (prose already on the row, no Workspace or Drawer inspector): omit navigateTo, hide the Repeat (or its Grid/Stack/Section) with showWhen "!selectedId", put Chip setValue result views plus bound DataText/KeyValue in a sibling Section showWhen "selectedId" with a ghost Back Button clearItem true (no navigateTo). Do not append History markdown below an always-visible Repeat. Workspace and Drawer keep the collection visible — do not hide navigator or primary with !selectedId; inspectorWhen or Drawer showWhen "selectedId" reveals the inspect region. Do not invent a second fetch for a field already on the row. When the list API only returns an id, keep the fetch-one detail onLoad instead. History cards bind item.keyword / item.client only inside Repeat. Results after Generate still echo the home form names ({targetKeyword}, {clientBrand}), not those history keys ({keyword}, {client}).',
  ...
  'Navigation: Tabs items are newline-separated "Label|path" lines with distinct page paths for two or more peer destinations (Generator|home, History|history); set activePath to the current path. Same-page result views (Enhanced Article / Coverage / Gap Analysis / Recommendations on one Generator page or History detail after View) are Chip setValue with one Chip per view — put a horizontal row above the bound panels; a left vertical rail only when the brief explicitly asks for it. The host paints the selected Chip and does not drop History selectedId. Do not emit catalog Tabs for those labels. Do not emit two Chips per label. A search hero omits Tabs. Detail and progress pages are reached with NavLink/navigateTo and offer a Back NavLink.',
...
export const ARENA_GENERATIVE_UI_INTERACTION_RULES = [
  ...
  'Empty copy: bound Table, Repeat, Calendar, and KeyValue use emptyText (defaults: "No results" for Table, Repeat, and Calendar, "No details" for KeyValue). A DataText fallback is the empty copy for prose. Customise emptyText when the brief names the collection ("No matching articles"). Do not add a second Text or Alert for that.',
  'Result pages: when onSuccess.navigate is set, bind the destination Table/Repeat/Stat/Chart/KeyValue/DataText. Same-page generate (no results page): bind those components below WorkingCard on the task page.',
] as const
