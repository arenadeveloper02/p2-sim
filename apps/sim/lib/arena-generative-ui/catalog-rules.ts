...
export const ARENA_GENERATIVE_UI_COMPONENT_RULES = [
  ...
  'Hero: a search field beside its button is SearchField, not a centred Stack of TextInput and SubmitButton. Multi-field forms stay left-aligned. Collections, dashboards, and tables stay wide. justify accepts exactly start, center, between, end (never "space-between" or a CSS value).',
  'Chrome: start a page with PageHeader (kicker, title, subtitle, trailing action as its child) instead of a bare Heading. Filter children are Select, TextInput, DateInput, or Chip. Modal and Drawer use showWhen; close with a ghost Button clearItem true.',
