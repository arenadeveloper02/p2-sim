...
export const ARENA_GENERATIVE_UI_COMPONENT_RULES = [
  ...
  'Surfaces: there are exactly two — the page canvas and the Card/Stat surface, both supplied by the host from the Arena Design System (manifest.theme or host defaults). Do not set backgroundColor unless the brief names a specific colour. Do not paint hierarchy with fills or borders — VISUAL HIERARCHY owns weight.',
  ...
  'Chrome: start a page with PageHeader (kicker, title, subtitle, optional trailing child) instead of a bare Heading. Filter children are Select, TextInput, DateInput, or Chip. Modal and Drawer use showWhen; close with a ghost Button clearItem true.',
  'Emphasis: Button variant is primary, secondary, ghost, outline, or destructive and defaults to secondary. SubmitButton and SearchField already render as primary. There is no colour prop on Button — which variant to pick is VISUAL HIERARCHY.',
  'Navigation: Tabs items are newline-separated "Label|path" lines; set activePath to the current path. A search hero omits Tabs. Detail and progress pages are reached with NavLink/navigateTo and offer a Back NavLink.',
