...
export const ARENA_GENERATIVE_UI_ACTION_CONTRACT_PROMPT = [
  ...
  'during — The host disables that control, shows a spinner on it, and keeps the form and page visible. Do not emit ProgressSteps, Spinner, ProgressBar, or a disabled overlay. WorkingCard only when the brief named steps, estimate, or Cancel. Do not clear inputs. If onSuccess.navigate is set with navigateWhen immediate (default), the host navigates immediately — bind the destination region, not a loader on the form page the user left. If navigateWhen is success, stay on the form until the CTA succeeds; wait and errors stay on the form, then navigate.',
