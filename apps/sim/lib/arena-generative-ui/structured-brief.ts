...
const ARCHETYPE_RECIPES: Record<ArenaGenerativeArchetype, string> = {
  collection: [
    ...
    'Rules: Honour pages[].representation (list / table / cards / calendar / timeline). Bind collection data when bindings exist; dummy/local mode seeds 4–8 static Table rows, Repeat items, Calendar events, or Timeline events. Do not add search, filter, stats, or a detail page unless the blueprint listed them. Entity actions stay on the entity. Inspect is CAPABILITY inspect: same-page when pages[].regions.inspector or interaction.inspect is not navigate; a Detail page only when the sitemap already has one. Loading, empty, and error are host — set emptyText.',
  ].join('\n'),
  detail: [
