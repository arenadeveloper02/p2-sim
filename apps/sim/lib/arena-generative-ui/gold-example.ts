...
export const ARENA_GENERATIVE_UI_GOLD_EXAMPLE = [
  ...
  `Replace the actions apiKey values ("${GOLD_EXAMPLE_LOAD_API_KEY}", "${GOLD_EXAMPLE_API_KEY}", "${GOLD_EXAMPLE_RUN_API_KEY}") with declared API binding keys. Each SearchField/Form/Button actionId must be that same manifest.actions key — do not paraphrase ${GOLD_EXAMPLE_API_KEY} as company_search. Drop manifest.actions and every onLoad entirely when no bindings were declared.`,
