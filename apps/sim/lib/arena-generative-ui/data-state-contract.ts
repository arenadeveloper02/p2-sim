...
export const ARENA_GENERATIVE_UI_DATA_STATE_PROMPT = [
  ...
  'loading — Bind statePath. The host skeletons that region in its catalog shape (table, card, stat, text). Chat paints three bouncing dots on the left while its action is in flight. Do not emit a page-level Skeleton, Spinner, ProgressBar, or typing indicator as the wait. WorkingCard only when the brief named steps, estimate, or Cancel.',
  ...
  'partial — Leave regions that already have data visible. Do not invent placeholder rows or fake metrics. The host marks incomplete regions busy with Updating; do not add a second ProgressBar.',
