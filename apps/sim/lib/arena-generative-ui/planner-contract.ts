...
export const PLANNER_CORE_PROMPT = [
  ...
  'You decide: application scope, sitemap, page archetypes, composition (compose vs navigate vs local, plus navigateWhen), workspace regions, entity relationships, required capabilities, interaction model, data mode, representation, shell.',
  ...
  'DESIGN INTENT describes intent, not styling. Do emit productType (saas | analytics | crm | marketing | finance | productivity | content) plus density, tone, visualPriority, interactionStyle. Example: productType marketing, density roomy, tone editorial, visualPriority content, interactionStyle scannable. Do not specify exact colors, padding, font sizes, border radius, or component styling.',
  ...
  'BLUEPRINT ORDERING: (1) user job (2) complexity (3) required entities (4) required capabilities (5) composition decision — compose vs navigate vs local, then navigateWhen when replace (6) sitemap (7) page archetypes (8) workspace regions (9) interaction model (10) representation (11) data mode (12) shell (13) design intent. Do not let a component choice drive architecture.',
...
export const PLANNER_COMPOSITION_PROMPT = [
  ...
  'WHEN — compose vs navigate vs local: Ask whether the user must see both at once. Alongside / while keeping X visible / inspect without leaving / split view → compose on one Workspace page. Result replaces the current view (task submit then report) → navigate to another page. Emit composition.afterSubmit replace and composition.navigateWhen: immediate when the visitor should leave the form before the request resolves (streaming generate); success when they stay on the form until the API succeeds (JSON qualify/lookup — wait and errors stay on the form). Result stacks below the form on the same Generator view (below, not beside, not replace) → one task page with wait then results; not Workspace and not a two-column form beside an empty results pane. Create/edit/confirm is temporary → local interaction (dialog, drawer, inline), not a page and not a region. Parent/child entities do not auto-create extra routes — default is one Workspace page unless the user asked to leave the current view. Inspect navigates to a Detail page unless the user asked to keep the collection visible. Do not create a Results page merely because an operation returns data if the result must stay on the task page; if the result replaces the task, two pages are correct. A named History tab or history API is a separate collection page even when Generator is one view. Always emit composition { afterSubmit, navigateWhen?, inspect, history, mutations } before inventing pages.',
  'COMPOSITION DECISION ORDER (before sitemap): (1) user job (2) required composition units (3) must they remain visible together — compose, navigate, or local (4) if replace: navigateWhen immediate or success (5) if compose: which regions, which archetype per region, what coordination (6) only then page count, paths, shell.',
...
export const PLANNER_COMPOSITION_EXAMPLES_CORE = [
  'COMPOSITION EXAMPLES: Todo → one collection page; create/complete are capabilities, not pages or regions. CRM → collection pages plus a Detail page (inspect navigates) unless the prompt asked for a persistent inspector. Competitor analysis / prospect research / multi-source briefing → task page then results page (replace, not compose). A briefing is not a KPI grid — do not pick dashboard for research prose plus source lists. Google Ads / LinkedIn / KPI monitor → dashboard. Project management → one home Workspace page; navigator = project collection, primary = task collection, inspector = task detail; interaction.selection and inspect name the flows; no second page for tasks.',
].join('\n')
