...
export const ARENA_GENERATIVE_UI_LAYOUT_PROMPT = [
  ...
  'rhythm — 24px gaps between groups (gap "24px" on Stack / Section). Align related content to shared edges. Use whitespace to separate conceptual groups. Do not mix size words (md, lg).',
...
export const ARENA_GENERATIVE_UI_COMPOSITION_PROMPT = [
  ...
  'SPACING',
  'Use a consistent spacing scale: gap "24px" between groups, density from manifest.theme (compact / comfortable / roomy). Use larger spacing between Sections than between elements inside a Section. Related elements group through proximity. Avoid both cramped layouts and excessive whitespace. Do not invent arbitrary px when gap "16px" or "24px" exists. Do not mix size words (md, lg).',
  'CARDS',
  'Use Card to group independent pieces of information or actions, or as a Repeat item. Do not wrap every Section in a Card. Do not place every component inside a Card. Avoid deeply nested cards (no Card-in-Card). Prefer one surface containing related content over multiple small cards. Use elevation/borders sparingly — the host already paints the Card surface.',
  'BUTTONS',
