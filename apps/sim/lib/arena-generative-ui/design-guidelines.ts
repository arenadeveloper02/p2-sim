...
export const ARENA_GENERATIVE_UI_LAYOUT_PROMPT = [
  ...
  'forms — Multi-field Form and SearchField: Section "narrow", left-aligned, inside a Card. The host paints that Card surface if omitted and stretches Form to fill it. surface "none" only when the brief asks for flush / no container. Do not wrap Filter or Toolbar. Do not run a form the full 1280px.',
  'columns — At most two primary content columns (Columns main+sidebar, or Grid columns 2 for form fields and for Repeat card collections). Honour a brief that asks for two cards per row: Grid columns 2 wrapping Repeat (History run cards). Grid columns 3 for a denser Repeat catalog. Do not use three peer chrome columns except Workspace (navigator + primary + inspector; navigator and inspector are supporting, not second mains). Do not put a generate form beside an empty results pane — stack wait then results below the form, or navigate.',
...
export const ARENA_GENERATIVE_UI_COMPOSITION_PROMPT = [
  ...
  'VISUAL LANGUAGE',
  'Create a clean, modern, professional product UI. Prioritize clarity and hierarchy over decoration. Use visual restraint; avoid unnecessary gradients, extra elevation, borders, icons, and decorative elements. Cards use the host rest shadow; do not invent additional shadows or wrap Card in Card. Every visual element must serve a functional or informational purpose. Maintain consistent visual language across the entire application.',
  ARENA_GENERATIVE_UI_LAYOUT_PROMPT,
  ARENA_GENERATIVE_UI_HIERARCHY_PROMPT,
  'TYPOGRAPHY',
  'Use a consistent type scale. PageHeader.title is visually distinct from section and Card headings. Heading h3 is the quieter in-card section title. Body (Text, DataText) prioritizes readability; tone default / muted / tertiary / brand is the ink scale. Supporting metadata has reduced emphasis. Avoid excessive font-size variation. Never use typography alone to communicate an important state (bind emptyText / host banner).',
  'COLOR',
  ...
  'CARDS',
  'Use Card to group independent pieces of information or actions, or as a Repeat item. variant "default" is the bordered host surface with rest shadow; "muted" is a muted fill with the same shadow. Form and SearchField sit in a Card — PageHeader stays outside. Do not wrap every Section in a Card. Do not place every component inside a Card. Avoid deeply nested cards (no Card-in-Card). Prefer one surface containing related content over multiple small cards. Do not invent extra elevation — the host already paints Card rest shadow.',
  'BUTTONS',
  ...
  'DATA VISUALIZATION',
  'Chart is the primary visualization when the bound collection is a numeric series (bar, line, area, pie) with axes or slices. Sparkline is only the compact trend under a Stat — not a full chart. Never generate a decorative Chart or Sparkline without statePath (or dummy categories + values). Include labels, units, and context when the brief names them. Avoid excessive visual decoration. Stat only from layoutPlan / outputSchema hostKeys.',
  'ICONS',
