...
function conversationTypes(params: SlackListChannelsParams): string {
  ...
  const isManagedOAuth = params.credentialType === 'managed_oauth'
  if (isManagedOAuth || resolveBooleanParam(params.includeDMs, 'Include direct messages', false)) {
    types.push('im')
  }
  if (
    isManagedOAuth ||
    resolveBooleanParam(params.includeGroupDMs, 'Include group direct messages', false)
  ) {
    types.push('mpim')
  }
  return types.join(',')
...
> = async (params, signal) => {
  ...
  const channels = parsed.channels.map((conversation) => {
    const mapped = mapSlackConversation(conversation)
    const isIm = Boolean(mapped.is_im)
    const isMpim = Boolean(mapped.is_mpim)
    return {
      ...mapped,
      name: mapped.name || (isIm ? `dm:${mapped.user || ''}` : isMpim ? 'group_dm' : mapped.name),
      ...(isIm || isMpim ? { is_private: true } : {}),
    }
  })
