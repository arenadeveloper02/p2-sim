...
import { executeSlackReadMessages } from '@/lib/internal/slack/read-messages'
...
export { executeSlackReadMessages }
...
async function postSlackMessage(
  ...
  return requestSlackApi({
    ...
    body: {
      ...
      ...(input.thread_ts ? { thread_ts: input.thread_ts } : {}),
      ...(input.blocks?.length ? { blocks: input.blocks } : {}),
      ...(input.link_names ? { link_names: input.link_names } : {}),
      ...(input.unfurl_links ? { unfurl_links: input.unfurl_links } : {}),
      ...(input.unfurl_media ? { unfurl_media: input.unfurl_media } : {}),
    },
    signal,
  })
}
