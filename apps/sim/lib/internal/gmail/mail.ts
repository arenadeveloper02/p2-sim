...
function resolveMailBody(input: MailInput): { body: string; contentType: 'text' | 'html' } {
  if (!('isHtml' in input) || !input.isHtml) {
    return { body: input.body, contentType: input.contentType || 'text' }
  }
  if (isValidHtml(input.body)) {
    return { body: input.body, contentType: 'html' }
  }
  return { body: renderAgentResponseToString(input.body), contentType: 'html' }
}
...
async function buildRawMessage(
  ...
  const { body: processedBody, contentType } = resolveMailBody(input)
