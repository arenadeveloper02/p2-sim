...
> = async (
  ...
  if (params.profile_id) {
    ...
    const sessionResult = await createSessionWithProfile(params.profile_id, apiKey, signal)
  ...
  try {
    ...
    const response = await fetchBrowserUse('/tasks', apiKey, {
      method: 'POST',
      body: requestBody,
      signal,
    })
    ...
    const shareUrl =
      ...
      (finalSessionId ? await createShareUrl(finalSessionId, apiKey, signal) : null)
