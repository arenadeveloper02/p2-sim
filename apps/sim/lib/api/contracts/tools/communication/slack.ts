...
export const slackSendMessageBodySchema = z
  .object({
    ...
    link_names: z.boolean().optional().nullable(),
    unfurl_links: z.boolean().optional().nullable(),
    unfurl_media: z.boolean().optional().nullable(),
  })
  .refine((data) => data.channel || data.userId, {
    message: 'Either channel or userId is required',
  })

const slackReadMessagesEmptyStringToUndefined = (val: unknown) => {
...
export const slackReadMessagesBodySchema = z
  .object({
    ...
    fromDate: z.preprocess(
      slackReadMessagesEmptyStringToUndefined,
      z.string().optional().nullable()
    ),
    toDate: z.preprocess(slackReadMessagesEmptyStringToUndefined, z.string().optional().nullable()),
