...
/**
 * Clears payer-pool usage caches. Tests must call this between cases that
 * reuse the same user or subscription, because the copilot-turn cache otherwise
 * returns the previous result.
 */
export function resetUsageMonitorCaches(): void {
  usageStatusCache.clear()
  serverSideUsageLimitsCache.clear()
}

function usageMonitorCacheKey(
  userId: string,
  preloadedSubscription?: UsageLimitSubscription | null,
  preloadedBillingContext?: BillingContext
): string {
  return `${userId}:${preloadedSubscription?.referenceId ?? 'auto'}:${preloadedBillingContext?.billingPeriod.start.toISOString() ?? 'auto'}`
}
...
export async function checkUsageStatus(
  ...
  const cacheKey = usageMonitorCacheKey(userId, preloadedSubscription, preloadedBillingContext)
  const cached = readUsageMonitorCache(usageStatusCache, cacheKey)
  if (cached) return cached

  const result = await checkUsageStatusUncached(
    userId,
    preloadedSubscription,
...
export async function checkServerSideUsageLimits(
  ...
  const result = await checkServerSideUsageLimitsUncached(
    userId,
    preloadedSubscription,
    preloadedBillingContext
  )
