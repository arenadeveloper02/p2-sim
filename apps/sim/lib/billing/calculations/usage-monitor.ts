 ...
 *
 * Hosted-only and independent of the pooled org limit. The workspace lookup
 * keeps this legacy gate compatible with callers that do not yet carry an
 * immutable billing attribution snapshot.
 */
...
export async function checkActorUsageLimits(
  ...
  if (workspaceId) {
    const memberUsage = await checkOrgMemberUsageLimit(userId, workspaceId)
    if (memberUsage.isExceeded) {
      return { isExceeded: true, message: memberUsage.message, scope: 'member' }
    }
  }
