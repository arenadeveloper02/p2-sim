...
export interface ApiFallbackUploadOptions {
  workspaceId?: string
  /** Required by the server for the `org-logos` context, which rejects the upload without it. */
...
export async function uploadViaApiFallback(
  ...
  const { path, key } = await uploadViaApiFallbackWithMetadata(file, context, {
    workspaceId,
    organizationId,
  })
  return { path, key }
