...
describe('uploadViaApiFallbackWithMetadata', () => {
  ...
  it('forwards organizationId, which the server requires for org-logos', async () => {
    mockFetch.mockResolvedValue(
      new Response(JSON.stringify({ path: '/api/files/serve/org-logos%2Flogo.png' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      })
    )
    ...
    const result = await uploadViaApiFallback(file, 'org-logos', undefined, 'organization-1')

    expect(result).toEqual({ path: '/api/files/serve/org-logos%2Flogo.png', key: undefined })

    const request = mockFetch.mock.calls[0]?.[1] as RequestInit
    const formData = request.body as FormData
    expect(formData.get('context')).toBe('org-logos')
    expect(formData.get('organizationId')).toBe('organization-1')
