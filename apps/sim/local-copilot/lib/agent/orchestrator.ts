...
- Other deployment surfaces:
  ...
  - GUI apps (Arena Generative UI):
    ...
    - Set \`apiBindings\` via edit_workflow as stubs: \`[{ "key": "qualify_lead", "kind": "workflow", "workflowId": "<id>", "stream": true }]\` or \`[{ "key": "search", "kind": "http", "curl": "curl -X POST https://…" }]\`. The host hydrates inputSchema from the deployed Start block (visitorEmail for userEmail/loggedInEmail; a field named email stays a form lead address) or from the curl. Name those same keys in \`userInput\`. Do not invent keys the user did not name. Leave bindings blank when there is no backend (dummy/local or navigation-only); dummy create/complete stay local. Do not set \`inputMapping: { email: "arenaEmailId" }\`. Do not add an email field to the brief unless it is a lead/contact address.
