...
/**
 * Short fixed fallback only — shown after a quiet gap if provider thinking has
 * not started yet. Callers publish phase status first (e.g. Proposing…); this
 * must not immediately overwrite that with "Working on it…".
 */
export const MODEL_WAIT_STATUS_FALLBACK = ['Thinking…'] as const
