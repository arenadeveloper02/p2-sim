...
export async function executeTool(
  ...
  try {
    ...
    if (normalizedToolId === 'load_skill') {
      ...
      const skillsResolverModulePath = '@' + '/executor/handlers/agent/skills-resolver'
      const { resolveSkillContent } = await import(skillsResolverModulePath)
...
async function executeToolRequest(
  ...
  try {
    ...
    if (typeof endpointUrl === 'object' && endpointUrl !== null && '_errorResponse' in endpointUrl) {
    ...
    if (!endpointUrl) {
      throw new Error(`Tool "${toolId}" returned an empty request URL`)
    }
