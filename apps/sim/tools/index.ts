...
import { withCallerOwnedDeadline } from '@/lib/core/utils/fetch-deadline'
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        if (isInternalRoute) {
           ...
           * for no effect. `withCallerOwnedDeadline` disarms Bun's 300s idle timer so the
           * AbortController above is the only bound there. Node's undici ignores `timeout:
           * false`.
           */
