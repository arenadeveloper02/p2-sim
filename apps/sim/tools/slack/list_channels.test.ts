...
describe('Slack list channels', () => {
  ...
  it('uses the internal operation boundary for a single page', () => {
    ...
    expect(slackListChannelsTool.oauth?.authoritativeParams).toEqual(['credentialType'])
  ...
  it('returns one page with conversation details and a cursor without fetching more', async () => {
    ...
    expect(result).toEqual({
      ...
      output: {
        ...
        ids: ['C123', 'D123', 'G123'],
        names: ['general', 'dm:U123', 'mpdm-one--two-1'],
        count: 3,
        hasMore: true,
        nextCursor: 'cursor-2',
      },
  ...
  it.each([undefined, 'oauth', 'service_account'] as const)(
    'does not request DM types for credential storage type %s',
  ...
  it('requests DMs and group DMs for credential-group user tokens', async () => {
    fetchMock.mockImplementation(async () =>
      slackResponse({ ok: true, channels: [], response_metadata: { next_cursor: '' } })
    )
  ...
  it('includes DMs when the caller opts in on a non-managed credential', async () => {
    fetchMock.mockResolvedValueOnce(
      slackResponse({ ok: true, channels: [], response_metadata: { next_cursor: '' } })
    )
