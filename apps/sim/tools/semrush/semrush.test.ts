...
const semrushTools = Object.fromEntries(
  Object.values(semrushExports)
    .filter(
      (value): value is ToolConfig =>
        Boolean(value) && typeof value === 'object' && 'id' in value && 'params' in value
    )
...
describe('semrush registry surface', () => {
  ...
  it('resolves every column code the tools request', () => {
    for (const [id, tool] of registered) {
      if (id === 'semrush_query' || id === 'semrush_organic_positions') continue
...
describe('semrush block alignment', () => {
  ...
  it('maps legacy generic-query operations onto dedicated tools', () => {
    expect(SemrushBlock.tools.config?.tool({ operation: 'url_organic' })).toBe(
      'semrush_url_organic_keywords'
    )
    expect(SemrushBlock.tools.config?.tool({ operation: 'domain_organic' })).toBe(
      'semrush_domain_organic_keywords'
    )
    expect(SemrushBlock.tools.config?.tool({ operation: 'domain_organic_organic' })).toBe(
      'semrush_domain_organic_competitors'
    )
    expect(SemrushBlock.tools.config?.tool({ operation: 'domain_rank' })).toBe(
      'semrush_domain_overview'
    )
    expect(SemrushBlock.tools.config?.tool({ operation: 'tracking_position_organic' })).toBe(
  ...
  it('shows a field for exactly the operations whose tool declares it', () => {
    ...
    for (const operation of access) {
      const tool = semrushTools[operation] as ToolConfig
