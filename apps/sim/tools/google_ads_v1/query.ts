...
export const googleAdsV1QueryTool: ToolConfig<GoogleAdsV1QueryParams, any> = {
  ...
  params: {
    accounts: {
      ...
      description: `Google Ads account key to query. Available accounts: ${Object.keys(GOOGLE_ADS_ACCOUNTS).join(', ')}`,
    },
    prompt: {
      type: 'string',
      required: true,
      visibility: 'user-or-llm',
      description:
        'Natural language prompt describing what data you want (e.g., "show campaign performance last 30 days", "keywords with low quality score", "search terms this week")',
    },
  },
  ...
  transformResponse: async (response: Response, params?: GoogleAdsV1QueryParams) => {
    try {
      ...
      logger.error('Google Ads V1 query failed', { error })
