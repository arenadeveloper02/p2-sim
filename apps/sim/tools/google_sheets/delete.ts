...
function resolveRowRange(params: GoogleSheetsDeleteParams): {
  ...
  // Priority: explicit start/end > rowNumber > range parsing
...
export const deleteTool: ToolConfig<GoogleSheetsDeleteParams, GoogleSheetsDeleteResponse> = {
  ...
  request: {
    ...
    body: async (params) => {
      ...
      // If no sheetId resolved, default to first sheet (0)
      if (sheetId === undefined) {
        sheetId = 0
      }

      // Sheets API uses zero-based startIndex, endIndex exclusive
