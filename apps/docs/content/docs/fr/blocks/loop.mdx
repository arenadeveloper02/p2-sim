...
<Callout type="info">
  Les blocs de boucle peuvent être imbriqués dans d'autres blocs de boucle, permettant une itération multidimensionnelle. Lorsque vous imbriquez des boucles :
  - Les boucles internes s'exécutent dans chaque itération de la boucle externe
  - Les variables de boucle comme `<loop.index>` font référence à la boucle conteneur la plus interne
  - Chaque boucle maintient son propre état d'itération de manière indépendante
  - Utilisez des boucles imbriquées pour traiter des structures de données multidimensionnelles ou effectuer des opérations itératives complexes
</Callout>
