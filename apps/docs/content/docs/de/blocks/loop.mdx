...
<Callout type="info">
  Schleifenblöcke können in andere Schleifenblöcke verschachtelt werden, was mehrdimensionale Iterationen ermöglicht. Bei verschachtelten Schleifen:
  - Innere Schleifen werden in jeder Iteration der äußeren Schleife ausgeführt
  - Schleifenvariablen wie `<loop.index>` beziehen sich auf die innerste enthaltende Schleife
  - Jede Schleife verwaltet ihren eigenen Iterationszustand unabhängig
  - Verwende verschachtelte Schleifen für die Verarbeitung mehrdimensionaler Datenstrukturen oder komplexe iterative Operationen
</Callout>
