...
<Callout type="info">
  Loop blocks can be nested inside other loop blocks, enabling multi-dimensional iteration. When you nest loops:
  - Inner loops execute within each iteration of the outer loop
  - Loop variables like `<loop.index>` refer to the innermost containing loop
  - Each loop maintains its own iteration state independently
  - Use nested loops for processing multi-dimensional data structures or performing complex iterative operations
</Callout>
