...
<Callout type="info">
  Los bloques de bucle pueden anidarse dentro de otros bloques de bucle, permitiendo iteración multidimensional. Cuando anidas bucles:
  - Los bucles internos se ejecutan dentro de cada iteración del bucle externo
  - Las variables de bucle como `<loop.index>` se refieren al bucle contenedor más interno
  - Cada bucle mantiene su propio estado de iteración de forma independiente
  - Usa bucles anidados para procesar estructuras de datos multidimensionales o realizar operaciones iterativas complejas
</Callout>
