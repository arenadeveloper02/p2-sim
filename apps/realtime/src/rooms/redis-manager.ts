...
import { healCorruptedWorkflowRoomKeys } from '@/rooms/workflow-room-keys'
...
export class RedisRoomManager implements IRoomManager {
  ...
  /**
   * Room keys must be Redis hashes. A STRING (or other type) at the same key —
   * e.g. from a shared cache collision or manual SET — causes WRONGTYPE on join
   * and leaves clients stuck retrying. Only workflow rooms need healing (the
   * healing function is keyed by workflowId); other room types start fresh.
   */
  private async ensureHealthyRoomKeys(room: RoomRef): Promise<void> {
    if (room.type !== 'workflow') return
    const result = await healCorruptedWorkflowRoomKeys(this.redis, room.id)
    if (!result.healed) {
      return
    }

    for (const entry of result.corrupted) {
      logger.warn('Deleted corrupted room Redis key before hash write', {
        room,
        key: entry.key,
  ...
  async addUserToRoom(room: RoomRef, socketId: string, presence: UserPresence): Promise<void> {
    try {
      await this.ensureHealthyRoomKeys(room)
