...
- **Release-batched sync:** each Actions run merges up to 6 upstream `vX.Y.Z:` tips (or fewer if that is all remaining); a successful complete dispatches the next unpaid batch in a separate job.
