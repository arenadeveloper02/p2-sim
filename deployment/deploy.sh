#!/bin/sh
# Stop app → maintenance on 3000/3002 → pull + build → start app → drop maintenance.
...
COMPOSE="-f docker-compose.test-env.yml -f deployment/docker-compose.deploy.yml"
...
docker compose $COMPOSE up -d --build --no-start db realtime simstudio
...
docker compose $COMPOSE up -d db realtime simstudio
