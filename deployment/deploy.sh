#!/bin/sh
# Maintenance serves deployment/maintenance.html on host 3000/3002 during git pull + image build.
...
docker build -f "$REPO_DIR/deployment/Dockerfile.maintenance" \
  -t p2-sim-maintenance "$REPO_DIR/deployment"
...
docker compose $COMPOSE_BASE run --rm migrations || true
