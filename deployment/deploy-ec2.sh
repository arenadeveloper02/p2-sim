...
docker build -f "$REPO_DIR/deployment/Dockerfile.maintenance" -t p2-sim-maintenance "$REPO_DIR/deployment"
...
docker rmi -f "p2-sim-simstudio" || true
...
docker rmi -f "p2-sim-realtime" || true
